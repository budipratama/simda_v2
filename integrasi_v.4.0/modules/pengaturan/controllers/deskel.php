...
class Deskel extends CI_Controller {
	...
	function datatable() {
		...
		$where_datatable = 'ms_urusan.tahun = \''.$tahun.'\' AND (skpd.skpd_status = \'Desa\' OR skpd.skpd_status= \'Kelurahan\')';
	...
	public function insert() {
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data Desa/Kelurahan baru telah berhasil ditambahkan.</div>');
	...
	public function check_insert($skpd_kd) {
		...
		if ($result == 0){
			...
			<strong>Error!</strong> <span>Kode Desa/Kelurahan yang Anda masukan sudah terdaftar.</span>
			</div>');
			return FALSE;
		}
	}
