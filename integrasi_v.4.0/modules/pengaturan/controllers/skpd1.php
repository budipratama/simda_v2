...
class Skpd extends CI_Controller {
	...
	public function cari() {
		...
		redirect('pengaturan/skpd/thn/'.$tahun);
	}
	...
	public function insert()
	{
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			redirect('pengaturan/skpd/thn/'.$tahun, 'refresh');
		}
	}
	...
	public function tampil_combobox_skpd_by_urusan(){
		$where1 = 'kode IN (\'16\')';
		$kode_skpd		= $this->uri->segment(4);
		if ($kode_skpd){
			$data_urusan = $this->Urusan_model->grid_all('urusan.kode, urusan.urusan', 'urusan.urusan', '', '', '', $where, array('skpd.skpd_kode'=>$kode_skpd));			
			combobox('db', $data_urusan, 'bbb_kode', 'kode', 'urusan', '', '', 'Pilih Urusan', 'class="form-control show-tick" data-live-search="true" tabindex="1" required="required"');
		} else {
