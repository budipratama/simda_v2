...
class Kelurahan extends CI_Controller {
	...
	function get_buttons($id)
	{
		...
		if ($admin_log['level_kode'] == 15 || $admin_log['level_kode'] == 1){
	...
	public function index()
	{
		...
		$container['sidebar']['dataset']['aktive_menu'] = 4;
		$container['content']['view']					= 'musrenbang/kelurahan/view';
		
		if ($admin_log['level_kode'] == 4 || $admin_log['level_kode'] == 15){
		...
		if ($admin_log['level_kode'] == 4){
			$deskel = $this->Skpd_model->get('skpd_kd', array('skpd_kode' => $admin_log['skpd_kode']));
			$container['content']['dataset']['deskel']		= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'Kelurahan'), array('skpd_kd'=>$deskel->skpd_kd));
		}
	...
	public function cari()
	{
		...
		redirect('musrenbang/kelurahan/hasil-pencarian/'.$tahun.'/'.$tipe.'/'.$skpd.'/'.$kecamatan.'/'.$deskel.'/'.$kegiatan);
	}
	...
	public function hasil_pencarian()
	{
		...
		if ($admin_log['level_kode'] == 4){
			...
			$where											= 'skpd_status IN (\'Kelurahan\')';
	...
	public function sync()
	{
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 4;
		$container['content']['view']					= 'musrenbang/kelurahan/sync';
		...
		if ($admin_log['level_kode'] == 4){
			...
			$waktuEntri	= $this->Tahapan_model->get('selesai', array('kode' => '3'));
			$tahapanSKPD = $this->Tahapan_skpd_model->count_all(array('skpd_kode' => $admin_log['skpd_kode'], 'tahapan_kode' => '3'));
		}
	...
	public function sync_process()
	{
		...
		redirect('musrenbang/kelurahan/sync_preview/'.$jenis_.'/'.$tahun_.'/'.$tipe_.'/'.$kecamatan_.'/'.$deskel_);
	}
			...
			if ($jenis_ == 1){
				if (get_server_status(parseurl($this->config->item('api_url')))){
					...
					if ($get_key){
						...
						$count_musrenbang = $this->rest->get('musrenbang_kelurahan/count/'.$tahun_.'/'.$tipe_);
						if ($count_musrenbang->success === true){
							$container['content']['dataset']['jumlah_data']	= $count_musrenbang->data;
							
							$this->rest->api_key($key, $name); 
							$grid_musrenbang = $this->rest->get('musrenbang_kelurahan/grid/'.$tahun_.'/'.$tipe_);
							$sama 				= 0;
						...
						if ($count_musrenbang > 0){
							...
							if ($grid_musrenbang){
								foreach($grid_musrenbang as $row_musrenbang){
									$this->rest->api_key($key, $name); 
									$get_musrenbang = $this->rest->get('musrenbang_kelurahan/get/'.$row_musrenbang->nomor);
				...
				if ($this->input->post('import_data')){
									...
									if (count($tmp_grid) > 0){
										...
										foreach($tmp_grid as $row_grid){
											if ($row_grid[2] == $deskel_kode){
												...
												$this->session->set_flashdata('error','<div class="alert alert-danger fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Error!</strong> File yang diupload salah. File tersebut bukan hasil musrenbang kelurahan yang Anda pilih.</div>');
												redirect('musrenbang/kelurahan/sync_preview/'.$jenis_.'/'.$tahun_.'/'.$tipe_.'/'.$kecamatan_.'/'.$deskel_);
											}
										}
					...
					redirect('musrenbang/kelurahan/sync_do/'.$jenis_.'/'.$tahun_.'/'.$tipe_.'/'.$kecamatan_.'/'.$deskel_.'/semua/'.$this->input->post('filename'));
				} else if ($this->input->post('btnimport_part')){
					redirect('musrenbang/kelurahan/sync_do/'.$jenis_.'/'.$tahun_.'/'.$tipe_.'/'.$kecamatan_.'/'.$deskel_.'/sebagian/'.$this->input->post('filename'));
				}
			} else if ($jenis_ == 4){
				$count_musrenbang = $this->Anggaran_model->count_all(array('anggaran.skpd_kode'=>$deskel_kode, 'anggaran.tahun'=>$tahun_, 'anggaran.tipe_kode'=>$tipe_));
				if ($count_musrenbang > 0){
					...
					if ($tmp_column[0] == 'kode'){
						if (count($tmp_grid) > 0){
							...
							if ($grid_musrenbang){
								...
								foreach($grid_musrenbang as $row_musrenbang){
									if ($sync == 'semua'){
										...
										if ($anggaran){
											...
											if ($row->foto){
												...
												if ($foto){
													foreach($foto as $row2){
														if ($row2){
															if (file_exists('./public/uploads/pictures/musrenbang_kelurahan/'.$row2)){
																unlink('./public/uploads/pictures/musrenbang_kelurahan/'.$row2);
															}
														}
													}
												}
											...
											if ($row->file){
												if (file_exists('./public/uploads/documents/musrenbang_kelurahan/'.$row->file)){
													unlink('./public/uploads/documents/musrenbang_kelurahan/'.$row->file);
												}
											}
										...
										if ($row_musrenbang->file){
											...
											file_put_content('./public/uploads/documents/musrenbang_kelurahan/'.$new_file, $row_musrenbang->file_);
										...
										if ($row_musrenbang->foto){
											...
											foreach($ds_foto_ as $row_foto){
												...
												$filename[] 	= $new_file;
												file_put_content('./public/uploads/pictures/musrenbang_kelurahan/'.$new_file, $row_foto);
												$i++;
											} 
								...
								redirect('musrenbang/kelurahan/sync', 'refresh');
							} else {
							...
							$this->session->set_flashdata('error','<div class="alert alert-danger fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Error!</strong> File yang diupload tidak ada data.</div>');
							redirect('musrenbang/kelurahan/sync_preview/'.$jenis_.'/'.$tahun.'/'.$tipe_.'/'.$kecamatan_.'/'.$deskel_);
						}
					} else {
						$this->session->set_flashdata('error','<div class="alert alert-danger fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Error!</strong> File yang diupload formatnya tidak sesuai.,</div>');
						redirect('musrenbang/kelurahan/sync_preview/'.$jenis_.'/'.$tahun.'/'.$tipe_.'/'.$kecamatan_.'/'.$deskel_);
		...
		if ($jenis_ && $tahun_ && $tipe_ && $sync && $nomor){
			if ($jenis_ == 1){
				...
				if ($get_key){
					...
					$this->rest->api_key($key, $name); 
					$count_musrenbang = $this->rest->get('musrenbang_kelurahan/get/'.$nomor);
					if ($count_musrenbang->success === true){
						if ($sync == 'semua'){
							$where_delete['anggaran.tahun']		= $tahun_;
							$where_delete['anggaran.tipe_kode']	= $tipe_;
							...
							$this->rest->api_key($key, $name); 
							$grid_musrenbang = $this->rest->get('musrenbang_kelurahan/get_full/'.$nomor);
							$sama = 0;
							if ($grid_musrenbang->success === true){
								...
								if ($row_musrenbang->file){
									...
									$new_file		=  $row_musrenbang->nomor. '' . $currenttimemilis . '' . str_shuffle($currenttimemilis) . '.' . end(explode(".", basename($row_musrenbang->file)));
									file_put_contents('./public/uploads/documents/musrenbang_kelurahan/'.$new_file, file_get_contents($this->config->item('api_media').'documents/musrenbang_kelurahan/'.$row_musrenbang->file));
									$insert['file'] = $new_file;
								} else {
									$insert['file'] = $row_musrenbang->file;
								}
								if ($row_musrenbang->foto){
									...
									foreach($ds_foto as $row_foto){
										...
										$filename[] 	= $new_file;
										file_put_contents('./public/uploads/pictures/musrenbang_kelurahan/'.$new_file, file_get_contents($this->config->item('api_media').'pictures/musrenbang_kelurahan/'.$row_foto));
										$i++;
									} 
									$insert['foto']					= implode(';', $filename);
								} else {
						...
						if ($grid_musrenbang){
							foreach($grid_musrenbang as $row_musrenbang){
								if ($sync == 'semua'){
									...
									$this->rest->api_key($key, $name); 
									$this->rest->post('musrenbang_kelurahan/delete', $param_delete);
									...
									if ($row_musrenbang->file){
										if (file_exists('./public/uploads/documents/musrenbang_kelurahan/'.$row_musrenbang->file)){
											$param_insert['file_']				= file_get_content(base_url("public/uploads/documents/musrenbang_kelurahan/".$row_musrenbang->file));
										}
									}
									$param_insert_	= array();
									...
									foreach($foto as $row_foto){
										if ($row_foto){
											if (file_exists('./public/uploads/pictures/musrenbang_kelurahan/'.$row_foto)){
												$param_insert_[$i]				= file_get_content(base_url("/public/uploads/pictures/musrenbang_kelurahan/".$row_foto));
												$i++;
											}
										}
									}
									...
									$this->rest->api_key($key, $name); 
									$this->rest->post('musrenbang_kelurahan/save', $param_insert);
				...
				foreach($grid_data as $row){
					...
					if ($row->foto){
						...
						foreach($foto as $row_foto){
							if ($row_foto){
								if (file_exists('./public/uploads/pictures/musrenbang_kelurahan/'.$row_foto)){
									$tmp_foto[$i]	= file_get_content(base_url("/public/uploads/pictures/musrenbang_kelurahan/".$row_foto));
									$i++;
								}
							}
						}
					...
					if ($row->file){
						if (file_exists('./public/uploads/documents/musrenbang_kelurahan/'.$row->file)){
							$tmp['file_'] 			= file_get_content(base_url("public/uploads/documents/musrenbang_kelurahan/".$row->file));;
						}
					} else {
						$tmp['file_'] 				= '';
					}
	...
	{	
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 4;
		$container['content']['view']					= 'musrenbang/kelurahan/list';
	...
	public function belanja_langsung()
	{
		...
		$waktuSekarang = date("Y-m-d H:i:s");
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			redirect('musrenbang/kelurahan/#warningEntri', 'refresh');
		} else {
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 4;
			$container['content']['view']					= 'musrenbang/kelurahan/belanja_langsung/add';
	...
	public function insert(){
		...
		if ($type == 'bl'){
			...
			$this->form_validation->set_rules('deskel_kode', 'Desa/Kelurahan', 'trim|required|xss_clean');
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$file = upload_file("file", "./public/uploads/documents/musrenbang_kelurahan/");
				if ($file){
					$insert['file']		= $file;
				}
				...
				if ($delete_file != ""){
					if (file_exists('./public/uploads/documents/musrenbang_kelurahan/'.$delete_file)){
						unlink('./public/uploads/documents/musrenbang_kelurahan/'.$delete_file);
					}
					$insert['file']		= '';
				}
				...
				if ($cpt > 0){				
					for($i=0; $i<$cpt; $i++)
					{
						...
						$file_name 						= upload_image_thumb('photo', './public/uploads/pictures/musrenbang_kelurahan/', '800x600');
						if ($file_name){
							$filename[] = $file_name;
						}
					}
				...
				if ($delete_foto){
					foreach($delete_foto as $row){
						if ($row){
							if (file_exists('./public/uploads/pictures/musrenbang_kelurahan/'.$row)){
								unlink('./public/uploads/pictures/musrenbang_kelurahan/'.$row);
							}
						}
					}
				}
				...
				$insert['nomor']			= nomorKegiatan($skpd->skpd_kode, $this->input->post('tahun'), 3, 1, $this->input->post('nomor'));
				...
				$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan anggaran belanja langsung telah berhasil ditambahkan.</div>');
				redirect('musrenbang/kelurahan', 'refresh');
			}
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$skpd = $this->Skpd_model->get('skpd.skpd_kode', array('skpd.skpd_kd'=>$this->input->post('deskel_kode')));
				$insert['skpd_kode']		= $skpd->skpd_kode;
				$insert['nomor']			= nomorKegiatan($skpd->skpd_kode, $this->input->post('tahun'), 3, 2, $this->input->post('nomor'));
				$insert['pelaksana_kode']	= $this->input->post('skpd_kode');
				$insert['tahun']			= $this->input->post('tahun');
				$insert['kegiatan']			= $this->input->post('kegiatan');
				$insert['lokasi_kode']		= $this->input->post('lokasi_kode');
				...
				$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan anggaran belanja tidak langsung telah berhasil ditambahkan.</div>');
				redirect('musrenbang/kelurahan', 'refresh');
			}
		}
	}
	...
	public function delete()
	{
		...
		if ($anggaran->status == 1){
			...
			if ($anggaran->file){
				if (file_exists('./public/uploads/documents/musrenbang_kelurahan/'.$anggaran->file)){
					unlink('./public/uploads/documents/musrenbang_kelurahan/'.$anggaran->file);
				}
			}
			...
			$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan telah berhasil dihapus.</div>');
		} else {
	...
	public function detail()
	{
		...
		if ($anggaran->tipe_kode == 1){
			...
			$container['content']['view']					= 'musrenbang/kelurahan/belanja_langsung/detail';
			...
			$container['content']['view']					= 'musrenbang/kelurahan/belanja_tidak_langsung/detail';
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'musrenbang/kelurahan/belanja_langsung/edit';
					...
					if ($admin_log['level_kode'] == 4){
						...
						$container['content']['dataset']['deskel']		= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', 'skpd_status IN (\'Kelurahan\')', array('skpd_kd'=>$anggaran_bl->kecamatan_kode));
					}
					$container['content']['dataset']['kesepakatan']	= $this->Kesepakatan_model->grid_all('kode, nama', 'kode', 'ASC');
					$container['content']['dataset']['sifat']		= $this->Sifat_model->grid_all('sifat_kode, sifat_nama', 'sifat_kode', 'ASC');
					...
					$container['content']['view']					= 'musrenbang/kelurahan/belanja_tidak_langsung/edit';
					...
					if ($admin_log['level_kode'] == 4){
						...
						$container['content']['dataset']['deskel']		= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', 'skpd_status IN (\'Kelurahan\')', array('skpd_kd'=>$anggaran_btl->kecamatan_kode));
					}
	...
	public function update(){
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan anggaran belanja langsung telah berhasil diperbaharui.</div>');
				redirect('musrenbang/kelurahan', 'refresh');
			}
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan anggaran belanja tidak langsung telah berhasil diperbaharui.</div>');
				redirect('musrenbang/kelurahan', 'refresh');
			}
		}
	}
	...
	public function belanja_tidak_langsung()
	{
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 4;
			$container['content']['view']					= 'musrenbang/kelurahan/belanja_tidak_langsung/add';
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'musrenbang/kelurahan/belanja_langsung/transfer';
					...
					$anggaran_btl									= $this->Anggaran_model->get('2','anggaran.*, anggaran_btl.biaya, anggaran_btl.volume, , anggaran_btl.penerima', array('anggaran.kode'=>$kode));
					$container['content']['view']					= 'musrenbang/kelurahan/belanja_tidak_langsung/transfer';
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				if ($file){
					...
					copy('./public/uploads/documents/musrenbang_kelurahan/'.$anggaran_->file, './public/uploads/documents/musrenbang_kecamatan/'.$anggaran_->file);
				}
				...
				if ($foto){
					foreach($foto as $row){
						if ($row){
							$filename[]	= $row;
							if (file_exists('./public/uploads/pictures/musrenbang_kelurahan/'.$row)){
								copy('./public/uploads/pictures/musrenbang_kelurahan/'.$row, './public/uploads/pictures/musrenbang_kecamatan/'.$row);
							}
						}
					}
				} 
				...
				$insert['sumber_nomor']		= nomorKegiatan($anggaran_->skpd_kode, $anggaran_->tahun, 3, 1, $nomor);
				...
				$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan anggaran belanja langsung telah berhasil ditransfer ke musrenbang kecamatan.</div>');
				redirect('musrenbang/kelurahan', 'refresh');
			}
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$insert['sumber_kode']		= 2;
				$insert['sumber_id']		= $kode;
				$insert['sumber_nomor']		= nomorKegiatan($anggaran_->skpd_kode, $anggaran_->tahun, 3, 2, $nomor);
				$insert['tahapan_kode']		= 4;
				$insert['tanggal']			= date("Y-m-d h:i:s");
				$insert['penambahan_kode']	= 1;
				$insert['admin_user']		= $admin_log['username'];
				...
				$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data hasil musrenbang kelurahan anggaran belanja tidak langsung telah berhasil ditransfer ke musrenbang kecamatan.</div>');
				redirect('musrenbang/kelurahan', 'refresh');
			}
		}
	}
	...
	public function tampil_combobox_deskel_by_kecamatan(){
		...
		$kode		= $this->uri->segment(4);
		if ($kode){
			echo '<label class="control-label col-md-3">Kelurahan</label>';
			echo '<div class="col-md-9">';
			$where											= 'skpd_status IN (\'Kelurahan\')';
			$like['skpd_kd']								= $kode;
			$data_skpd = $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', $where, $like);
			combobox('db', $data_skpd, 'deskel_kode', 'skpd_kd', 'skpd_nama', '', '', 'Semua Kelurahan', 'class="select2_category form-control"');
			echo '</div>';
		} else {
			echo '<label class="control-label col-md-3" for="deskel_kode">Kelurahan</label>
				<div class="col-md-9">
	...
	public function tampil_combobox_deskel_by_kecamatan2(){
		$admin_log = $this->auth->is_login_admin();
		echo '<label class="control-label">Kelurahan <span class="required">*</span> :</label>';
		$where											= 'skpd_status IN (\'Kelurahan\')';
		$like['skpd_kd']								= $this->uri->segment(4);
		$data_skpd = $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', $where, $like);
		combobox('db', $data_skpd, 'deskel_kode', 'skpd_kd', 'skpd_nama', '', '', 'Pilih Kelurahan', 'class="select2_category form-control"');
	...
	public function tampil_combobox_deskel_by_kecamatan4(){
		...
		if ($kode){
			...
			$like['skpd_kd']								= $kode;
			$data_skpd = $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', $where, $like);
			combobox('db', $data_skpd, 'deskel_kode', 'skpd_kd', 'skpd_nama', '', '', 'Semua Kelurahan', 'class="select2_category form-control" required="required"');
			echo '</div>';
		} else {
			echo '<label class="control-label col-md-3" for="deskel_kode">Kelurahan</label>
				<div class="col-md-9">
	...
	public function previewfile(){
		...
		if ($anggaran_->file != ""){
			echo '<iframe src="https://docs.google.com/gview?url='.base_url('public/uploads/documents/musrenbang_kelurahan/'.$anggaran_->file).'&embedded=true" width="100%" height="100%" style="border: none;"></iframe>';
		} else {
			echo 'Tidak ada file';
		}
	}
	...
	public function loadSkpd()
	{
		...
		echo "<thead>
					...
					if ($query->num_rows() > 0){
						...
						echo "
							<tr>
								<td style=\"cursor: pointer\" onclick=\"link('" . site_url('musrenbang/kelurahan/'.$tipe.'/'.$atribut->skpd_kode) . "')\">".$id."</td>
								<td style=\"cursor: pointer\" onclick=\"link('" . site_url('musrenbang/kelurahan/'.$tipe.'/'.$atribut->skpd_kode) . "')\">".$atribut->skpd_nama."</td>
								<td style=\"cursor: pointer\" onclick=\"link('" . site_url('musrenbang/kelurahan/'.$tipe.'/'.$atribut->skpd_kode) . "')\">".$atribut->skpd_kewenangan."</td>
								<td style=\"text-align:center;cursor: pointer\" onclick=\"link('" . site_url('musrenbang/kelurahan/'.$tipe.'/'.$atribut->skpd_kode) . "')\"><span class=\"label label-sm label-warning\">Isi Kegiatan</span></td>
							</tr>";
						$id++;
						} 
					} 
