...
class Murni extends CI_Controller {
	...
	public function belanja_langsung() 
	{
		...
		$this->form_validation->set_rules('eee_kode', 'Kd_rek_5', 'trim|required|xss_clean|callback_check_insert');
	...
	public function rincian()
	{
		$admin_log 	= $this->auth->is_login_admin();
		$kode 		= $this->uri->segment(4);
		$anggaran	= $this->Rka_model->getOnly('kode, kd_skpd', array('kode'=>$kode));
	
		$container['sidebar']['view']						= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] 	= 49;	
		$container['content']['view']						= 'rka/murni/belanja_langsung/rincian';
		
			$skpd_id										= 'skpd_kd IN ('.$anggaran->kd_skpd.')';
			$skpd = $this->Skpd_model->get('skpd_kode, skpd_kd, skpd_nama', $skpd_id, array('skpd_tahun' => $admin_log['tahun']));
			$container['content']['dataset']['skpd_kode']	= $skpd->skpd_kd;
			$container['content']['dataset']['skpd_nama']	= $skpd->skpd_nama;

			$container['rka']								= $this->Rka_model->bl($kode,true);
	...
	public function addr()
	{
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/addr';
			...
			$insert['keterangan']	= $this->input->post('aaa_kode');
			$insert['no_rinc']		= $this->input->post('no_kode');
			$insert['kd_belanja']	= $this->input->post('id_kode');
			$insert['kd_skpd']		= $this->input->post('sss_kode');
			$insert['kd_anggaran']	= $this->input->post('ggg_kode');
			$id_kode				= $this->input->post('id_kode');
			$insert['tipe_bl']	= 1;
			
			$query = $this->Rka_model->insertr($insert);			
			$this->session->set_flashdata('success','<div class="alert alert-info fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "RKA RINCIAN BELANJA" baru telah berhasil ditambahkan.</div>');
	...
	public function deleter()
	{
		$admin_log 	= $this->auth->is_login_admin(); {
			$kode 		= $this->uri->segment(4);
			$rincian	= $this->Rka_model->getOnlyr('kode, kd_belanja', array('kode'=>$kode));
			$this->Rka_model->delete4($this->uri->segment(4));
			$this->Rka_model->delete3($this->uri->segment(4));
			$this->session->set_flashdata('success','<div class="alert fresh-color alert-danger fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "RKA RINCIAN BELANJA" telah berhasil dihapus</div>');
			redirect('rka/murni/rincian/'.$rincian->kd_belanja, 'refresh');
		}
	}
	...
	public function sub() 
	{
		$admin_log 	= $this->auth->is_login_admin();
		$kode 		= $this->uri->segment(4);
		$anggaran 	= $this->Rka_model->getOnlyr('kode, kd_skpd', array('kode'=>$kode));
		...
		//	$sub = $this->Rka_model->getr('ms_rek_1.kd_rek_1, ms_rek_2.kd_rek_2, ms_rek_3.kd_rek_3, ms_rek_4.kd_rek_4, ms_rek_5.kd_rek_5, ms_rek_5.nm_rek_5', array('ta_belanja_rinc.kode' => $kode));
		//	$container['content']['dataset']['id_akun']		= $sub->kd_rek_1;
		//	$container['content']['dataset']['id_kelompok']	= $sub->kd_rek_2;
		//	$container['content']['dataset']['id_jenis']	= $sub->kd_rek_3;
		//	$container['content']['dataset']['id_obyek']	= $sub->kd_rek_4;
		//	$container['content']['dataset']['id_rincian']	= $sub->kd_rek_5;
		//	$container['content']['dataset']['nama_rincian']= $sub->nm_rek_5;
		
			$container['sub']								= $this->Rka_model->sub($kode,true);
			$container['rincian']							= $this->Rka_model->get_rincian($kode);
			$container['bls']								= $this->Rka_model->bls($kode,true);
		//	$container['rka']								= $this->Rka_model->bl($kode);

		///	$container['sum']								= $this->Rka_model->sum($kode,true);
		///	$container['jum']								= $this->Rka_model->jum($kode,true);
			
		//	$rka = $this->Rka_model->get('ta_belanja.*', array('ta_belanja.kode'=>$this->uri->segment(4)));		
		//	$container['content']['dataset']['kode']		= $rka->kode;
	...
	public function adds($list_id = null)
	{
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/adds';
			...
			$insert['kd_belanja']		= $this->input->post('id_kode');			
			$insert['kd_belanja_rinc']	= $this->input->post('kode');			
			$insert['satuan123']		= $this->input->post('bbb_kode');
			$insert['jml_satuan'] 		= $this->input->post('userMsg');
			$insert['nilai_rp']			= $this->input->post('thirdBox');
			$insert['total']			= $this->input->post('total_sum');
			$insert['kd_anggaran']		= $this->input->post('rincian_kode');
	...
	public function deletes() {
		$admin_log 	= $this->auth->is_login_admin(); {
			$kode 		= $this->uri->segment(4);
			$sub 		= $this->Rka_model->getOnlys('kode, kd_belanja_rinc', array('kode'=>$kode));
			...
			redirect('rka/murni/sub/'.$sub->kd_belanja_rinc, 'refresh');
		}
	}
