...
class Murni extends CI_Controller {
	...
	public function edit($id) {
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/edit';
			...
			$container['content']['dataset']['jenis']		= $this->Jenis_model->grid_all('kode, jenis_nama', 'jenis_nama', '', '', '');
			$container['content']['dataset']['obyek']		= $this->Obyek_model->grid_all('kode, obyek_nama', 'obyek_nama', '', '', '');
			$container['content']['dataset']['rincian']		= $this->Rincian_model->grid_all('kode, rincian_nama', 'rincian_nama', '', '', '');
			...
			redirect('rka/murni/belanja/'.$jenis->kelompok, 'refresh');
			}
		}
    }
	...
	public function rincian()
	{
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/rincian';		
			...
			$belanja										= $this->Anggaran_model->get('1','anggaran.kode, anggaran.tahun, anggaran.kegiatan, anggaran.catatan, anggaran.proposal, anggaran.verifikasi, anggaran.foto, anggaran.koordinat, kecamatan.skpd_nama as kecamatan_nama, deskel.skpd_nama as deskel_nama, anggaran.alamat, anggaran.rt, , anggaran.rw, anggaran_bl.urutan, anggaran_bl.apbd_kab, anggaran_bl.apbd_prov, anggaran_bl.apbn, anggaran_bl.sumberlain, anggaran_bl.perkiraan_maju, anggaran_bl.hp_ukur, anggaran_bl.hp_target, anggaran_bl.hp_satuan, sifat.sifat_nama, kesepakatan.nama as kesepakatan_nama, pelaksana.skpd_nama, visi.visi as visi_nama, misi.misi as misi_nama, prioritas.prioritas as prioritas_nama, tujuan.tujuan as tujuan_nama, sasaran.sasaran as sasaran_nama, indikator.indikator as indikator_nama, urusan.urusan as urusan_nama, program.program as program_nama, anggaran_bl.kh_ukur, anggaran_bl.kh_target, anggaran_bl.kh_satuan, anggaran_bl.hk_ukur, anggaran_bl.hk_target, anggaran_bl.hk_satuan', array('anggaran.kode'=>$kode));
			$container['content']['dataset']['kode'] 		= $belanja->kode;			
			$container['content']['dataset']['skpd'] 		= $belanja->skpd_nama;
	...
	public function addr()
	{
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/addr';
			...
			$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "BELANJA RINCIAN" baru telah berhasil ditambahkan.</div>');
	...
	public function sub()
	{
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/sub';		
			...
			$container['rka']								= $this->Rka_model->bl($kode);
			$container['rincian']							= $this->Rka_model->blr($kode);
	...
	public function adds($list_id = null)
	{
		...
		$container['content']['view']						= 'rka/murni/belanja_langsung/adds';
			...
			$this->session->set_flashdata('success','<div class="alert alert-success fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "SUB RINCIAN BELANJA" baru telah berhasil ditambahkan.</div>');
	...
	public function view() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			...
			//$container['content']['dataset']['tahun']		= $this->Tahun_model->grid_all('tahun', 'tahun', 'ASC', '', '', array('status'=>'1'));
			//$container['content']['dataset']['rka']		= $this->Rka_model->grid_all('anggaran_kode', 'anggaran_kode', 'ASC', '', '', array('tipe_kode'=>'1'));
			$container['content']['dataset']['kecamatan']	= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'Kecamatan'));
	...
	{	
		...
		if ($this->input->post('tahun')) { 
			...
			redirect('rka/murni/preview', 'refresh');
		}
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'rka/murni/belanja_tidak_langsung/edit';
					$container['content']['dataset']['kode'] 		= $anggaran_btl->kode;
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				if ($this->input->post('apbd_kab') <= $anggaran_sebelumnya){
					...
					redirect('rka/murni/#successUpdate', 'refresh');
				} else {
	...
	public function tampil_combobox_akun_by_kelompok(){
		...
		if ($akun_kode){
			...
			echo '<label class="control-label col-md-3" for="bbb_kode">Kelompok:</label>
				<div class="col-md-7"><select class="select2_category form-control" name="bbb_kode" id="bbb_kode" data-placeholder="Pilih Kelompok ..." tabindex="1"></select></div>';
		}
	}	
	...
	public function tampil_combobox_kelompok_by_jenis(){
		...
		if ($kelompok_kode){
			...
			echo '<label class="control-label col-md-3" for="ccc_kode">Jenis :</label>
				<div class="col-md-7"><select class="select2_category form-control" name="ccc_kode" id="ccc_kode" data-placeholder="Pilih Jenis ..." tabindex="1"></select></div>';
		}
	}
	...
	public function tampil_combobox_jenis_by_obyek(){
		...
		if ($jenis_kode){
			...
			echo '<label class="control-label col-md-3" for="ddd_kode">Obyek :</label>
				<div class="col-md-7"><select class="select2_category form-control" name="ddd_kode" id="ddd_kode" data-placeholder="Pilih Kelompok ..." tabindex="1"></select></div>';
		}
	}
	...
	public function tampil_combobox_obyek_by_rincian(){
		...
		if ($obyek_kode){
			...
			echo '<label class="control-label col-md-3" for="eee_kode">Rincian :</label>';
			echo '<div class="col-md-7">';
				combobox('db', $data_rincian, 'eee_kode', 'kode', 'rincian_nama', '', '', 'Pilih Rincian ...', 'class="select2_category form-control" required="required"');
			echo '</div>';
		} else {
			echo '<label class="control-label col-md-3" for="eee_kode">Rincian :</label>
				<div class="col-md-7"><select class="select2_category form-control" name="eee_kode" id="eee_kode" data-placeholder="Pilih Jenis ..." tabindex="1"></select></div>';
		}
	}
