...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="data_table">	
	...
	<td style="text-align:center;" colspan="3" rowspan="2" width="55">JUMLAH<br>(Rp)</td>		
		...
		<tr style="font-weight:bold; background:#B0C4DE ;">
			...
			<td colspan="5" style="text-align:right;">2</td>
		</tr>
		
	<?php
	$jenis_urusan 	= array('1'=>'63 Belanja Pegawai', '2'=>'64 Belanja Barang dan Jasa', '3'=>'65 Belanja Modal');
	for ($i=1; $i < 4; $i++){
		?>
		...
		<?php		
		$query_id = $this->db->query("SELECT rka.kode as id_rka,
		rka.no, obyek.nomor as obyek_no,
		obyek.obyek_nama as obyek_nama,
		SUM(rka_sub.total) as totalRKA
		FROM rka 
		LEFT JOIN obyek ON rka.obyek=obyek.kode
		LEFT JOIN rka_sub ON rka.kode=rka_sub.kode
		WHERE rka.tipe_kode= '1' 
		AND rka.jenis='".$jenis_urusan[$i]."'
		...
		if ($jumlah_id > 0){
			...
			<?php
				$query_rka = $this->db->query("SELECT rincian.nomor as rincian_no,
			...
			?>
					<tr>
						...
						<td colspan="5" style="text-align:right;"><?php echo rupiah1 (); ?></td>
					</tr>
			...
			?>
					...
					<?php
						...
						rka_sub.uraian, SUM(total) as totalRKA
						FROM rka_sub						
	...
	?>
		...
		<tr style="background-color:#B0C4DE; height:25px; font-weight:bold;">
			...
			<td style="text-align:right;" colspan="5"><?php echo rupiah1 ($total_rka); ?></td>
        </tr>          
        </table>
    </td>
  </tr>
