<!-- Main Content -->
<?php $query_data = mysql_query("SELECT skpd.skpd_nama as nama_skpd, program.program as id_program, anggaran.kegiatan as id_kegiatan FROM rka INNER JOIN skpd ON rka.skpd=skpd.skpd_kode INNER JOIN program ON rka.program=program.kode INNER JOIN anggaran ON rka.anggaran_kode=anggaran.kode WHERE rka.tipe_kode= '1' AND rka.anggaran_kode='".$anggaran."' ORDER BY rka.kode ASC"); $data = mysql_fetch_array($query_data); $nama_skpd = $data[nama_skpd]; $program = $data[id_program]; $kegiatan = $data[id_kegiatan]; $query_jumlah = mysql_query("SELECT SUM(total) FROM rka_sub WHERE rka_sub.tipe_kode= '1' AND rka_sub.anggaran_kode='".$anggaran."' ORDER BY rka_sub.kode ASC"); $data = mysql_fetch_array($query_jumlah); $jumlah = $data[0]; ?>
							...
							<ul class="header-dropdown m-r--5">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a title="Kembali" href="<?php echo base_url(); ?>rka/murni/sub/<?php echo $kode; ?>"><i class="material-icons">reply</i> Kembali</a></li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
									...
									<form action="<?php echo site_url('rka/murni/preview/'.$rka);?>" class="horizontal-form" method="post">
										<div class="form-body">
											<div class="row">
												...
												<input type="hidden" name="anggaran_kode" value="<?php echo $anggaran; ?>" />
										...
										<div class="form-actions">
											<div class="col-md-offset-9">
												...
												<a href="<?php echo base_url(); ?>rka/murni/sub/<?php echo $kode; ?>" class="btn btn-default">Kembali</a>
											</div><br>	
										</div>	
									</form>
								</div>
