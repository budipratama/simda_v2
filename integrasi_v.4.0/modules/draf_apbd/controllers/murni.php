...
class Murni extends CI_Controller {
	...
	function datatable()
	{
		...
		->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, anggaran.kode, anggaran.kegiatan, anggaran.alamat, anggaran.pelaksana_kode, anggaran.status, anggaran.sumber_kode, anggaran.perubahan_status_kode FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, anggaran WHERE ('.$where_datatable.') ORDER BY anggaran.kode DESC) anggaran LEFT JOIN skpd ON anggaran.pelaksana_kode=skpd.skpd_kode LEFT JOIN status ON anggaran.status=status.status_kode LEFT JOIN sumber ON anggaran.sumber_kode=sumber.sumber_kode LEFT JOIN perubahan_status ON anggaran.perubahan_status_kode=perubahan_status.perubahan_status_kode');
	...
	public function index()
	{
		...
		$container['content']['view']					= 'draf_apbd/murni/view';
	...
	public function cari()
	{
		...
		redirect('draf-apbd/murni/hasil-pencarian/'.$tahun.'/'.$tipe.'/'.$skpd.'/'.$kecamatan.'/'.$deskel.'/'.$kegiatan);
	}
	...
	public function belanja_langsung()
	{
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 34;
			$container['content']['view']					= 'draf_apbd/murni/belanja_langsung/add';
	...
	public function insert(){
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				redirect('draf-apbd/murni/#successInsert', 'refresh');
			}
	...
	public function delete()
	{
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			//if ($anggaran->status == 1){
				...
				$this->Anggaran_model->update('anggaran', array('perubahan_status_kode'=>'4'), array('kode' => $kode)); 
				...
				redirect('draf-apbd/murni/#successDelete', 'refresh');
				
			//} else {
				
			//	redirect('draf-apbd/murni/#warningTransfer', 'refresh');
			// }

			//redirect('draf-apbd/murni', 'refresh');
		}
	}
	...
	public function detail()
	{
		...
		if ($anggaran->tipe_kode == 1){
			...
			$container['content']['view']					= 'draf_apbd/murni/belanja_langsung/detail';
			...
			$container['content']['view']					= 'draf_apbd/murni/belanja_tidak_langsung/detail';
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'draf_apbd/murni/belanja_langsung/edit';
					if ($admin_log['level_kode'] == 5 || $admin_log['level_kode'] == 4){
						$skpd = $this->Skpd_model->get('skpd_kode, skpd_nama', array('skpd_kode' => $admin_log['skpd_kode']));
						$container['content']['dataset']['skpd_kode']	= $skpd->skpd_kode;
						$container['content']['dataset']['skpd_nama']	= $skpd->skpd_nama;
					...
					$container['content']['view']					= 'draf_apbd/murni/belanja_tidak_langsung/edit';
					$container['content']['dataset']['kode'] 		= $anggaran_btl->kode;
	...
	public function update(){
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$where_anggaran = "anggaran.kode =  '".$kode."'";
				$anggaran		= $this->db->query("SELECT anggaran_bl.apbd_kab FROM anggaran LEFT JOIN anggaran_bl ON anggaran.kode = anggaran_bl.anggaran_kode WHERE ".$where_anggaran)->row();
				
				if ($anggaran->apbd_kab == $this->input->post('apbd_kab')){
					$perubahan_status_kode = 1;
				} else if ($anggaran->apbd_kab < $this->input->post('apbd_kab')){
					$perubahan_status_kode = 2;
				} else if ($anggaran->apbd_kab > $this->input->post('apbd_kab')){
					$perubahan_status_kode = 3;
				} 
				...
				$update['tahapan_kode']		= 14;
				$update['tanggal']			= date("Y-m-d h:i:s");
				$update['penambahan_kode']	= 1;
				$update['perubahan_status_kode'] = $perubahan_status_kode;
				...
				redirect('draf-apbd/murni/#successUpdate', 'refresh');
	...
	public function belanja_tidak_langsung()
	{
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 34;
			$container['content']['view']					= 'draf_apbd/murni/belanja_tidak_langsung/add';
			$container['content']['dataset']['tahun']		= $this->Tahun_model->grid_all('tahun', 'tahun', 'ASC', '', '', array('status'=>'1'));
			$container['content']['dataset']['skpd']		= $this->Skpd_model->grid_all('skpd_kode, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'SKPD'));
			$container['content']['dataset']['kecamatan']	= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'Kecamatan'));
			$header['admin_log']							= $admin_log;
