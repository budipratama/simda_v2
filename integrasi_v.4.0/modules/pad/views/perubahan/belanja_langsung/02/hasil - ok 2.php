...
<body>
<table width="850" border="0" cellspacing="0" cellpadding="4" align="center" class="wrapper">
  <tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="data_table">	
	...
	FROM rka LEFT JOIN program ON rka.program=program.kode LEFT JOIN skpd ON rka.skpd=skpd.skpd_kode LEFT JOIN urusan ON rka.urusan=urusan.kode	LEFT JOIN anggaran ON rka.anggaran_kode=anggaran.kode
	LEFT JOIN anggaran_bl ON rka.anggaran_kode=anggaran_bl.kode	WHERE rka.tipe_kode= '1' AND rka.kode='".$laporan_kode."' AND rka.tahun='".$laporan_tahun."'	AND rka.anggaran_kode='".$laporan_anggaran."' GROUP BY rka.kode ORDER BY rka.kode ASC");
	...
	<?php
	$query_jenis = $this->db->query("SELECT rka.kode as id_kode, jenis.jenis_nama as id_jenis, jenis.no as no_jenis
		FROM rka
		LEFT JOIN jenis ON rka.jenis=jenis.kode
		...
		$jumlah_jenis		= $query_jenis->num_rows();
		
		$query_tj 	= $this->db->query("SELECT rka.kode as id_rka, rka_sub.total, SUM(rka_sub.total) as totalRKA FROM rka_sub LEFT JOIN rka ON rka.kode=rka_sub.rka WHERE rka_sub.tipe_kode= '1' AND rka.tahun='".$laporan_tahun."' AND rka.anggaran_kode='".$laporan_anggaran."' AND rka.kode GROUP BY rka.kode ORDER BY rka.kode ASC");
		$data_tj 	= $query_tj->result();
		
		if ($jumlah_jenis > 0){
			foreach($data_jenis as $row_jenis) {				
		?>
		<tr style="font-weight:bold; background:#DCDCDC ;">
			<td align="center">5.2.<?php echo strtoupper($row_jenis->no_jenis);?></td>
			<td colspan="3"><?php echo strtoupper($row_jenis->id_jenis);?></td>
			<td colspan="4" style="text-align:right;">
			<?php if($data_tj): foreach($data_tj as $task):  echo rupiah1 ($task->totalRKA);  endforeach;  endif; ?>
			</td>
		</tr>
		<?php			
		$query_id = $this->db->query("SELECT rka.kode as id_rka, rka.no, obyek.nomor as obyek_no, obyek.obyek_nama as obyek_nama, SUM(rka_sub.total) as totalRKA FROM rka LEFT JOIN obyek ON rka.obyek=obyek.kode LEFT JOIN rka_sub ON rka.kode=rka_sub.kode WHERE rka.tipe_kode= '1' AND rka.tahun='".$laporan_tahun."' AND rka.anggaran_kode='".$laporan_anggaran."' GROUP BY rka.kode ORDER BY rka.kode ASC");
