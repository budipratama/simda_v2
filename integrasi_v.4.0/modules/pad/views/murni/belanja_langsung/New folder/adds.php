<!-- Main Content -->
<?php $query_data = mysql_query("SELECT skpd.skpd_nama as id_skpd, program.program as id_program, anggaran.kegiatan as id_kegiatan,

anggaran_bl.apbd_kab as id_kab, anggaran_bl.apbd_prov as id_prov, anggaran_bl.apbn as id_apbn, anggaran_bl.sumberlain as id_sumberlain 

FROM rka INNER JOIN skpd ON rka.skpd=skpd.skpd_kode 
INNER JOIN program ON rka.program=program.kode 
INNER JOIN anggaran ON rka.anggaran_kode=anggaran.kode   
INNER JOIN anggaran_bl ON rka.anggaran_kode=anggaran_bl.anggaran_kode   
WHERE rka.tipe_kode= '1' 
AND rka.anggaran_kode='".$rincian->anggaran_kode."' ORDER BY rka.kode ASC"); 
$data = mysql_fetch_array($query_data);
$skpd = $data[id_skpd]; 
$program = $data[id_program];
$kegiatan = $data[id_kegiatan]; 
$kab = $data[id_kab];    
$prov = $data[id_prov];    
$apbn = $data[id_apbn];    
$sumberlain = $data[id_sumberlain];    


$query_jumlah = mysql_query("SELECT SUM(total) as total FROM rka_sub WHERE rka_sub.tipe_kode= '1' AND rka_sub.anggaran_kode='".$rincian->anggaran_kode."' ORDER BY rka_sub.kode ASC"); 
                            ...
                            <div class="row clearfix">
                                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
									...
									<div class="body">
										<div class="row clearfix">
										<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
											<form action="<?php echo site_url('rka/murni/adds');?>" id="form_validation" class="form-horizontal" method="post" name="autoSumForm" onSubmit="return validasi()"><br>
											...
											<input type="hidden" id="stok" value="<?php echo $kab+$prov+$apbn+$sumberlain-$jumlah;?>">
