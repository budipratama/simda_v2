...
<tr>
   <td style='text-align:center; font-size:7pt; font-weight:bold; border:1px solid #000;'colspan='3'>PEMERINTAH KABUPATEN BEKASI<br>TAHUN ANGGARAN : $laporan_tahun</td>
	   	...
	   	if ($laporan_program == semua && $laporan_kegiatan == semua){
			...
			$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.program as id_program, program.nomor as no_program, SUM(rka_sub.total) as total, anggaran_bl.hp_ukur as hpu_anggaran, anggaran_bl.hp_target as hpt_anggaran, anggaran_bl.hp_satuan as hps_anggaran FROM rka_sub INNER JOIN rka ON rka_sub.rka=rka.kode INNER JOIN skpd ON rka.skpd=skpd.skpd_kode INNER JOIN urusan ON rka.urusan=urusan.kode INNER JOIN program ON rka.program=program.kode INNER JOIN anggaran ON rka.anggaran_kode=anggaran.kode INNER JOIN anggaran_bl ON rka.anggaran_kode=anggaran_bl.kode WHERE rka.tahun='".$laporan_tahun."' AND rka.tipe_kode='".$laporan_belanja."' AND rka.skpd='".$laporan_skpd."' AND rka.program='".$laporan_program."' ORDER BY rka_sub.kode ASC"); 
			...
			$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.program as id_program, program.nomor as no_program, anggaran.kegiatan as id_kegiatan, anggaran.nomor as no_anggaran, SUM(rka_sub.total) as total, anggaran_bl.hp_ukur as hpu_anggaran, anggaran_bl.hp_target as hpt_anggaran, anggaran_bl.hp_satuan as hps_anggaran, anggaran_bl.kh_ukur as khu_anggaran, anggaran_bl.kh_target as kht_anggaran, anggaran_bl.kh_satuan as khs_anggaran, anggaran_bl.hk_ukur as hku_anggaran, anggaran_bl.hk_target as hkt_anggaran, anggaran_bl.hk_satuan as hks_anggaran FROM rka_sub INNER JOIN rka ON rka_sub.rka=rka.kode INNER JOIN skpd ON rka.skpd=skpd.skpd_kode INNER JOIN urusan ON rka.urusan=urusan.kode INNER JOIN program ON rka.program=program.kode INNER JOIN anggaran ON rka.anggaran_kode=anggaran.kode INNER JOIN anggaran_bl ON rka.anggaran_kode=anggaran_bl.kode WHERE rka.tahun='".$laporan_tahun."' AND rka.tipe_kode='".$laporan_belanja."' AND rka.skpd='".$laporan_skpd."' AND rka.program='".$laporan_program."' AND rka.anggaran_kode='".$laporan_kegiatan."' ORDER BY rka_sub.kode ASC"); 
			$data = mysql_fetch_array($query_jumlah); $id_urusan = $data[id_urusan]; $nama_urusan = $data[nama_urusan]; $jenis_urusan = $data[jenis_urusan]; $id_skpd = $data[id_skpd]; $no_skpd = $data[no_skpd]; $lokasi = $data[alamat_skpd]; $status = $data[status_skpd]; $id_program = $data[id_program]; $no_program = $data[no_program]; $id_kegiatan = $data[id_kegiatan]; $no_anggaran = $data[no_anggaran]; $jumlah = $data[total]; $hpu_anggaran = $data[hpu_anggaran]; $hpt_anggaran = $data[hpt_anggaran]; $hps_anggaran = $data[hps_anggaran]; $khu_anggaran = $data[khu_anggaran]; $kht_anggaran = $data[kht_anggaran]; $khs_anggaran = $data[khs_anggaran]; $hku_anggaran = $data[hku_anggaran]; $hkt_anggaran = $data[hkt_anggaran]; $hks_anggaran = $data[hks_anggaran]; $rupiah = rupiah1($jumlah);
			$terbilang = Terbilang($jumlah);
			$skpd = strtoupper($id_skpd);
			...
			$where_data ['rka.tipe_kode']	= $laporan_belanja;
...
if ($jumlah_data > 0){
	...
	for ($i=1; $i < 4; $i++){
		...
		if ($this->Rka_model->count_bl($where_jenis) > 0){
			...
			if ($laporan_program == semua && $laporan_kegiatan == semua){
				$query_belanja	= mysql_query("SELECT rka.kode as id_rka, SUM(rka_sub.total) as totalRKA FROM rka_sub INNER JOIN rka ON rka_sub.rka=rka.kode WHERE rka.jenis='".$jenis_belanja[$i]."' AND rka.tipe_kode='".$laporan_belanja."' AND rka.tahun='".$laporan_tahun."' AND rka.skpd='".$laporan_skpd."'");
	...
	if ($laporan_program == semua && $laporan_kegiatan == semua){
			...
			$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.program as id_program, program.nomor as no_program, SUM(rka_sub.total) as total FROM rka_sub INNER JOIN rka ON rka_sub.rka=rka.kode INNER JOIN skpd ON rka.skpd=skpd.skpd_kode INNER JOIN urusan ON rka.urusan=urusan.kode	INNER JOIN program ON rka.program=program.kode WHERE rka.tahun='".$laporan_tahun."' AND rka.tipe_kode='".$laporan_belanja."' AND rka.skpd='".$laporan_skpd."' AND rka.program='".$laporan_program."' ORDER BY rka_sub.kode ASC"); 
			...
			$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.program as id_program, program.nomor as no_program, anggaran.kegiatan as id_kegiatan, anggaran.nomor as no_anggaran FROM rka_sub INNER JOIN rka ON rka_sub.rka=rka.kode INNER JOIN skpd ON rka.skpd=skpd.skpd_kode INNER JOIN urusan ON rka.urusan=urusan.kode INNER JOIN program ON rka.program=program.kode INNER JOIN anggaran ON rka.anggaran_kode=anggaran.kode WHERE rka.tahun='".$laporan_tahun."' AND rka.tipe_kode='".$laporan_belanja."' AND rka.skpd='".$laporan_skpd."' AND rka.program='".$laporan_program."' AND rka.anggaran_kode='".$laporan_kegiatan."' ORDER BY rka_sub.kode ASC"); 
