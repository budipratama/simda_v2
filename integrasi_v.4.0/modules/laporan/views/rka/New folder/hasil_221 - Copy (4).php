<?php
	if ($laporan_kegiatan == semua){
			$query_jumlah = mysql_query("SELECT skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, SUM(rka_sub.total) as total
			FROM rka_sub 
			...
			$data = mysql_fetch_array($query_jumlah); $jumlah = $data[total]; $lokasi = $data[alamat_skpd]; $status = $data[status_skpd];
		} else {
			$query_jumlah = mysql_query("SELECT skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, 
		...
		<tr>
	   	...
	   	if ($laporan_kegiatan == semua){
			$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan
			FROM rka_sub 
			INNER JOIN rka ON rka_sub.rka=rka.kode
			INNER JOIN skpd ON rka.skpd=skpd.skpd_kode
			...
			$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.program as id_program, program.nomor as no_program, anggaran.kegiatan as id_anggaran
			FROM rka_sub 
			INNER JOIN rka ON rka_sub.rka=rka.kode
			INNER JOIN skpd ON rka.skpd=skpd.skpd_kode
			INNER JOIN urusan ON rka.urusan=urusan.kode
			...
			AND rka_sub.anggaran_kode='".$laporan_kegiatan."'
			ORDER BY rka_sub.kode ASC"); 
			$data = mysql_fetch_array($query_jumlah); $id_urusan = $data[id_urusan]; $nama_urusan = $data[nama_urusan]; $jenis_urusan = $data[jenis_urusan]; $id_skpd = $data[id_skpd]; $no_skpd = $data[no_skpd]; $no_program = $data[no_program]; $id_program = $data[id_program]; $id_anggaran = $data[id_anggaran];
			...
			$strhtml .= "<table style='font-family: serif; border:1px solid #000;'>
				...
				<tr>
					...
					<th style='text-align:left; font-size:12pt; font-weight:0; width:800px'>$id_anggaran</th>
				</tr> 
				</table>";
		}
