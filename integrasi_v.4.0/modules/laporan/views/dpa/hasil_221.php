...
$strhtml0 .= "<br><br><table style='font-family: serif;'>
	...
	if ($laporan_program == semua && $laporan_kegiatan == semua){
		...
		SUM(anggaran_kas.anggaran) as total,
		...
		$query_jumlah = mysql_query("SELECT skpd.skpd_nomor as no_skpd, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd,
		urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.nomor as no_program, program.program as id_program,
		SUM(anggaran_kas.anggaran) as total,
		tipe.no as no_tipe, tipe.tipe_nama as nama_tipe,
		anggaran_bl.hp_ukur as hpu_anggaran, anggaran_bl.hp_target as hpt_anggaran, anggaran_bl.hp_satuan as hps_anggaran
		FROM anggaran_kas 
		INNER JOIN rka ON anggaran_kas.rka=rka.kode
		INNER JOIN anggaran_bl ON anggaran_kas.anggaran_kode=anggaran_bl.anggaran_kode
		INNER JOIN program ON anggaran_kas.program=program.kode
		INNER JOIN skpd ON anggaran_kas.skpd=skpd.skpd_kode
		INNER JOIN urusan ON rka.urusan=urusan.kode
		INNER JOIN tipe ON rka.sumber=tipe.tipe_kode
		WHERE rka.tahun='".$laporan_tahun."'
		AND anggaran_kas.tipe_kode='".$laporan_belanja."'
		AND anggaran_kas.skpd='".$laporan_skpd."'
		AND anggaran_kas.program='".$laporan_program."'
		ORDER BY anggaran_kas.kode ASC"); 
		$data = mysql_fetch_array($query_jumlah); 
		$id_urusan = $data[id_urusan]; $nama_urusan = $data[nama_urusan]; $jenis_urusan = $data[jenis_urusan]; $id_skpd = $data[id_skpd]; $no_skpd = $data[no_skpd]; $lokasi = $data[alamat_skpd]; $status = $data[status_skpd]; $jumlah = $data[total]; $no_tipe = $data[no_tipe]; $nama_tipe = $data[nama_tipe]; $no_program = $data[no_program]; $id_program = $data[id_program];
		...
		urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, program.nomor as no_program, program.program as id_program,
		SUM(anggaran_kas.anggaran) as total,
		tipe.no as no_tipe, tipe.tipe_nama as nama_tipe, anggaran.nomor as no_anggaran, anggaran.kegiatan as id_anggaran,
		anggaran_bl.hp_ukur as hpu_anggaran, anggaran_bl.hp_target as hpt_anggaran, anggaran_bl.hp_satuan as hps_anggaran,
		...
		FROM anggaran_kas 
		INNER JOIN rka ON anggaran_kas.rka=rka.kode
		INNER JOIN anggaran ON anggaran_kas.anggaran_kode=anggaran.kode
		INNER JOIN anggaran_bl ON anggaran_kas.anggaran_kode=anggaran_bl.anggaran_kode
		INNER JOIN program ON anggaran_kas.program=program.kode
		INNER JOIN skpd ON anggaran_kas.skpd=skpd.skpd_kode
		INNER JOIN urusan ON rka.urusan=urusan.kode
		...
		AND anggaran_kas.skpd='".$laporan_skpd."'
		AND anggaran_kas.program='".$laporan_program."'
		AND anggaran_kas.anggaran_kode='".$laporan_kegiatan."'
		ORDER BY anggaran_kas.kode ASC"); 
		$data = mysql_fetch_array($query_jumlah); 
		$id_urusan = $data[id_urusan]; $nama_urusan = $data[nama_urusan]; $jenis_urusan = $data[jenis_urusan]; $id_skpd = $data[id_skpd]; $no_skpd = $data[no_skpd]; $lokasi = $data[alamat_skpd]; $status = $data[status_skpd]; $jumlah = $data[total]; $no_tipe = $data[no_tipe]; $nama_tipe = $data[nama_tipe]; $no_program = $data[no_program]; $id_program = $data[id_program]; $no_anggaran = $data[no_anggaran]; $id_anggaran = $data[id_anggaran];
		$hpu_anggaran = $data[hpu_anggaran]; $hpt_anggaran = $data[hpt_anggaran]; $hps_anggaran = $data[hps_anggaran];
	...
	if ($laporan_program == semua && $laporan_kegiatan == semua){		
		...
		if ($laporan_program == semua && $laporan_kegiatan == semua){		
			$strhtml .= "</table><table style='font-family: serif; border:1px solid #000;'>
				<tr>
			...
			$where_data ['rka.tipe_kode']	= $laporan_belanja;
				...
				foreach ($this->Anggaran_kas_model->grid_all('1', 'anggaran_bl.kode as kode_bl, anggaran_bl.kh_ukur as khu_anggaran, anggaran_bl.kh_target as kht_anggaran, anggaran_bl.kh_satuan as khs_anggaran, anggaran_bl.hk_ukur as hku_anggaran, anggaran_bl.hk_target as hkt_anggaran, anggaran_bl.hk_satuan as hks_anggaran', 'anggaran_bl.kode', 'ASC', '', '', $where_data, '', 'anggaran_kas.anggaran_kode') as $datarow){
					$strhtml .= "<table style='font-family: serif; border:1px solid #000;'>
						<tr>
	...
	for ($i=1; $i < 4; $i++){
		...
		if ($this->Anggaran_kas_model->count_bl($where_jenis) > 0){
			...
			if ($laporan_program == semua && $laporan_kegiatan == semua){
				$query_belanja = $this->db->query("SELECT SUM(anggaran_kas.anggaran) as totalRKA FROM anggaran_kas LEFT JOIN rka ON anggaran_kas.rka=rka.kode WHERE rka.jenis='".$jenis_belanja[$i]."' AND rka.tipe_kode='".$laporan_belanja."' AND rka.tahun='".$laporan_tahun."' AND rka.skpd='".$laporan_skpd."'");
			...
			foreach ($this->Anggaran_kas_model->grid_all('1', 'obyek.kode as kode_obyek, obyek.nomor as nomor_obyek, obyek.obyek_nama as nama_obyek, SUM(rka_sub.total) as totalRKA', 'obyek.no', 'ASC', '', '', $where_jenis, '', 'rka.obyek') as $row){
				...
				if ($this->Anggaran_kas_model->count_bl($where_urusan) > 0){
				...
				$strhtml .= "<table style='font-family: serif; border:1px solid #000;'>
					...
					foreach ($this->Anggaran_kas_model->grid_all('1', 'rincian.kode as kode_rincian, rincian.nomor as nomor_rincian, rincian.rincian_nama as nama_rincian, SUM(rka_sub.total) as totalRKA', 'rincian.kode', 'ASC', '', '', $where_urusan, '', 'rincian.kode') as $subrow){
					...
					$strhtml .= "<table style='font-family: serif; border:1px solid #000;'>
						...
						foreach ($this->Anggaran_kas_model->grid_all('1', 'rka_rincian.kode as kode_rincian, rka_rincian.no as nomor_rincian, rka_rincian.uraian as nama_rincian, SUM(rka_sub.total) as totalRKA', 'rka_rincian.kode', 'ASC', '', '', $where_rincian, '', 'rka_rincian.kode') as $rincianrow){
						...
						$strhtml .= "<table style='font-family: serif; border:1px solid #000;'>
							...
							foreach ($this->Anggaran_kas_model->grid_all('1', 'rka_sub.kode as kode_sub, rka_sub.no as no_sub, rka_sub.uraian as nama_sub, rka_sub.volume, rka_sub.satuan, rka_sub.harga, rka_sub.total', 'rka_sub.kode', 'ASC', '', '', $where_sub, '', 'rka_sub.kode') as $rsubrow){
...
$mpdf=new mPDF( '',                          // mode (default '')
                ...
                5, 5, 5, 21, 5, 5,  //(margins) left, right, top, bottom, HEADER, FOOTER
                'L');
