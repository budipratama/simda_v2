...
header("Content-Disposition: attachment;Filename=Laporan_KUA_PPAS_Perubahan_".date("d_m_Y_H_i_s").".xls");
}
?>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=Windows-1252">
<title>LAPORAN KUA & PPAS PERUBAHAN KABUPATEN BEKASI</title>
<style>
@page {
	margin:0px;
	width:330mm;
...
<tr>
	<td colspan="3">
	<h2>DAFTAR KUA & PPAS PROGRAM DAN KEGIATAN PRIORITAS PERUBAHAN TAHUN <?php echo $laporan_tahun;?></h2>
...
<tr>
    ...
    <tr style="border-bottom:#999 solid 2px;">
		...
		<th colspan="3">Plafon Anggaran</th>
		<th rowspan="3" width="80">Status</th>
    </tr>
    <tr style="border-bottom:#999 solid 2px;">
		<th colspan="3">Hasil Program</th>
		<th colspan="3">Keluaran Kegiatan</th>
		<th colspan="3">Hasil Kegiatan</th>
		<th rowspan="2" width="110">Sebelum Perubahan</th>
		<th rowspan="2" width="110">Sesudah Perubahan</th>
		<th rowspan="2" width="110">Selisih</th>
    </tr>
    <tr style="border-bottom:#999 solid 2px;">
...
if ($jumlah_data > 0){
	...
	for ($i=1; $i <= 2; $i++){
		...
		<tr style="font-weight:bold; background:#9F0;">
			...
			<td colspan="18">URUSAN <?php echo strtoupper($jenis_urusan[$i]);?></td>
		</tr>
			<?php
			...
			foreach ($this->Anggaran_model->grid_all('1', 'urusan.kode as urusan_kode, urusan.nomor as urusan_nomor, urusan.urusan as urusan_nama', 'urusan.nomor', 'ASC', '', '', $where_jenis, '', 'anggaran_bl.urusan_kode') as $row){
				...
				if ($this->Anggaran_model->count_bl($where_urusan) > 0){
					...
					<tr style="font-weight:bold;font-style:italic;">
						...
						<td colspan="18"><?php echo $row->urusan_nama;?></td>
					</tr>
					<?php
				}
				...
				<tr style="font-weight:bold; background:#FF0;">
            		...
            		<td colspan="18" style="padding-left:10px;"><?php echo $subrow->skpd_nama;?></td>
        		</tr>
					<?php
					...
					if ($this->Skpd_model->count_all(array('skpd_kode'=>$subrow->skpd_kode, 'urusan'=>$row->urusan_kode)) == 1){
						...
						foreach ($this->Anggaran_model->grid_all('1', 'SUM(anggaran_bl.apbd_kab) as subjumlah_apbdkab, program.kode as program_kode, program.nomor as program_nomor, program.program as program_nama', 'program.nomor', 'ASC', '', '', $where_skpd_rutin, '', 'program.kode') as $subsubrow){
							...
							if ($this->Anggaran_model->count_bl($where_program_rutin) > 0){
								...
								$total_sebelum	= $total_sebelum + $subsubrow->subjumlah_apbdkab;
							...
							foreach ($this->Anggaran_model->grid_all('1', 'anggaran.kegiatan, anggaran.alamat, anggaran_bl.hp_ukur, anggaran_bl.hp_target, anggaran_bl.hp_satuan, anggaran_bl.kh_ukur, anggaran_bl.kh_target, anggaran_bl.kh_satuan, anggaran_bl.hk_ukur, anggaran_bl.hk_target, anggaran_bl.hk_satuan, anggaran_bl.apbd_kab AS apbd_sebelum, anggaran_bl.apbd_kab, sifat.sifat_nama, urusan.urusan as urusan_nama, sasaran.sasaran as sasaran_nama, prioritas.prioritas as prioritas_nama, program.program as program_nama, indikator.indikator as indikator_nama', 'anggaran.kegiatan', 'ASC', '', '', $where_program_rutin, '', 'anggaran.kode') as $kegiatanrow){
							...
							<tr>
                                ...
                                <td style="text-align:right;"><?php echo rupiah3($kegiatanrow->apbd_sebelum);?></td>
                                <td style="text-align:right;"><?php echo rupiah3($kegiatanrow->apbd_kab);?></td>
                                <td style="text-align:right;">0</td>
                                <td align="center"><i>Tetap</i></td>
							</tr>
					...
					foreach ($this->Anggaran_model->grid_all('1', 'SUM(anggaran_bl.apbd_kab) as subjumlah_apbdkab, program.kode as program_kode, program.nomor as program_nomor, program.program as program_nama', 'program.nomor', 'ASC', '', '', $where_skpd, '', 'anggaran_bl.program_kode') as $subsubrow){
...
?>
	<tr style="font-weight:bold;font-size:14px;">
        ...
        <th style="text-align:right;"><?php echo rupiah3($total_sebelum);?></th>
        <th style="text-align:right;">0</th>
        <th align="center">Tetap</th>
    </tr>
	</table>
    </td>
    </tr>
