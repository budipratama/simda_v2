...
class Pra_apbd extends CI_Controller {
	...
	{	
		...
		$container['content']['view']					= 'laporan/pra_apbd/view';
	...
	{	
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 19;
		$container['content']['view']					= 'laporan/pra_apbd/preview';
		...
		$container['content']['dataset']['jenis_laporan']	= $this->input->post('jenis_laporan');
		...
		$sess_laporan = array(
			...
			'laporan_kategori'  => $this->input->post('jenis_laporan'),
		...
		if ($this->input->post('tahun')) { 
			...
			redirect('laporan/pra-apbd/preview', 'refresh');
		}
	...
	{	
		...
		$laporan['laporan_kategori']	= $sess_laporan['laporan_kategori'];
		...
		if ($laporan['laporan_jenis'] == '1'){
			if ($laporan['laporan_kategori'] == '1'){
				$this->load->view('laporan/pra_apbd/hasil', $laporan);
			} else {
				$this->load->view('laporan/pra_apbd/hasil_parsial', $laporan);
			}
		} else {
			$this->load->view('laporan/pra_apbd/hasil_perubahan', $laporan);
		}
	} 
