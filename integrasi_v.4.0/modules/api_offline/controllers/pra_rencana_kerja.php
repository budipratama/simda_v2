<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Pra_rencana_kerja extends REST_Controller {
		...
		public function grid_get()
		{
			...
			$pelaksana 	= ($this->uri->segment(5))?$this->uri->segment(5):'skpd';
			$kecamatan 	= ($this->uri->segment(6))?$this->uri->segment(6):'kecamatan';
			$deskel 	= ($this->uri->segment(7))?$this->uri->segment(7):'deskel';
				...
				if ($this->input->post('nomor') && $this->input->post('skpd_kode')){
					...
					if ($cpt > 0){				
						for($i=0; $i<$cpt; $i++)
						{
							...
							$file_name 						= upload_image_thumb('photo', './public/uploads/pictures/pra_rencana_kerja/', '800x600');
							if ($file_name){
								$filename[] = $file_name;
							}
						}
