...
class Murni extends CI_Controller {
	...
	public function index() {
		...
		$container['sidebar']['dataset']['aktive_menu'] = 62;
		$container['content']['view']					= 'anggaran_kas/murni/view';
	...
	public function cari() {
		...
		redirect('anggaran-kas/murni/hasil-pencarian/'.$tahun.'/'.$tipe.'/'.$skpd.'/'.$kecamatan.'/'.$deskel.'/'.$kegiatan);
	}
	...
	public function detail() {
		...
		if ($anggaran->tipe_kode == 1){
			...
			$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/detail';
			...
			$container['content']['view']					= 'anggaran_kas/murni/belanja_tidak_langsung/detail';
	...
	public function view() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 62;
			$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/view';
			$container['content']['dataset']['kecamatan']	= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'Kecamatan'));
	...
	public function delete() {
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				redirect('anggaran-kas/murni/#successDelete', 'refresh');
			} else {
				redirect('anggaran-kas/murni/#warningTransfer', 'refresh');
			}
	...
	public function belanja() {
		...
		if ($anggaran->tipe_kode == 1){			
			...
			$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/belanja';
			$container['content']['dataset']['kode'] 		= $anggaran_bl->kode;
			$container['content']['dataset']['tahun'] 		= $anggaran_bl->tahun;
			$container['content']['dataset']['skpd'] 		= $anggaran_bl->skpd_nama;
			$container['content']['dataset']['visi'] 		= $anggaran_bl->visi_nama;
			...
			$anggaran_btl									= $this->Anggaran_model->get('2','anggaran.kode, anggaran.tahun, anggaran.kegiatan, anggaran.catatan, anggaran.proposal, anggaran.verifikasi, anggaran.foto, anggaran.koordinat, kecamatan.skpd_nama as kecamatan_nama, deskel.skpd_nama as deskel_nama, anggaran.alamat, anggaran.rt, , anggaran.rw, anggaran_btl.volume, anggaran_btl.biaya, anggaran_btl.penerima, pelaksana.skpd_nama', array('anggaran.kode'=>$kode));
			$container['content']['view']					= 'anggaran_kas/murni/belanja_tidak_langsung/belanja';
			$container['content']['dataset']['kode'] 		= $anggaran_btl->kode;
			$container['content']['dataset']['tahun'] 		= $anggaran_btl->tahun;
			$container['content']['dataset']['skpd'] 		= $anggaran_btl->skpd_nama;
			$container['content']['dataset']['kegiatan'] 	= $anggaran_btl->kegiatan;
	...
	public function rencana() {
		...
		$kode 		= $this->uri->segment(4);
		$anggaran 	= $this->Anggaran_model->getOnly('tipe_kode', array('kode'=>$kode));
		$this->form_validation->set_rules('kode_anggaran', 'Anggaran_kode', 'trim|required|xss_clean');
		$this->form_validation->set_rules('kode_total', 'Anggaran', 'trim|required|xss_clean');
		$this->form_validation->set_rules('kode_program', 'Program', 'trim|required|xss_clean');
		$this->form_validation->set_rules('kode_skpd', 'Skpd', 'trim|required|xss_clean');
		$this->form_validation->set_rules('kode', 'Rka', 'trim|required|xss_clean');
		
		if($this->form_validation->run() == FALSE) {
		$container['sidebar']['view']						= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu']		= 62;	
		$container['content']['view']						= 'anggaran_kas/murni/belanja_langsung/rencana';
		
			$rka = $this->Rka_model->get('rka.kode as id_kode, rka.anggaran_kode, skpd.skpd_nama, skpd.skpd_kode, anggaran.kegiatan as id_kegiatan, program.kode as program_kode, program.program as program_nama, akun.no as id_akun, kelompok.no as id_kelompok, jenis.no as id_jenis, obyek.no as id_obyek, rincian.no as id_rincian, rincian.rincian_nama as nama_rincian,', array('rka.kode'=>$this->uri->segment(4)));
			$container['content']['dataset']['id_kode']		= $rka->id_kode;
			$container['content']['dataset']['id_anggaran']	= $rka->anggaran_kode;
			$container['content']['dataset']['id_skpd']		= $rka->skpd_kode;
			$container['content']['dataset']['skpd']		= $rka->skpd_nama;
			$container['content']['dataset']['id_program']	= $rka->program_kode;
			$container['content']['dataset']['program']		= $rka->program_nama;
			$container['content']['dataset']['kegiatan']	= $rka->id_kegiatan;
			$container['content']['dataset']['akun']		= $rka->id_akun;
			$container['content']['dataset']['kelompok']	= $rka->id_kelompok;
			$container['content']['dataset']['jenis']		= $rka->id_jenis;
			$container['content']['dataset']['obyek']		= $rka->id_obyek;
			$container['content']['dataset']['rincian']		= $rka->nama_rincian;
			$container['content']['dataset']['rincian_']	= $rka->id_rincian;
			
			$anggaran_kas = $this->Rka_model->getak('anggaran_kas.*', array('anggaran_kas.rka'=>$this->uri->segment(4)));
			$container['content']['dataset']['jan_kode']	= $anggaran_kas->jan;
			$container['content']['dataset']['feb_kode']	= $anggaran_kas->feb;
			$container['content']['dataset']['mar_kode']	= $anggaran_kas->mar;
			$container['content']['dataset']['apr_kode']	= $anggaran_kas->apr;
			$container['content']['dataset']['mei_kode']	= $anggaran_kas->mei;
			$container['content']['dataset']['jun_kode']	= $anggaran_kas->jun;
			$container['content']['dataset']['jul_kode']	= $anggaran_kas->jul;
			$container['content']['dataset']['ags_kode']	= $anggaran_kas->ags;
			$container['content']['dataset']['sep_kode']	= $anggaran_kas->sep;
			$container['content']['dataset']['okt_kode']	= $anggaran_kas->okt;
			$container['content']['dataset']['nov_kode']	= $anggaran_kas->nov;
			$container['content']['dataset']['des_kode']	= $anggaran_kas->des;

		} else {
			$insert['anggaran_kode']= $this->input->post('kode_anggaran');
			$insert['anggaran']		= $this->input->post('kode_total');
			$insert['program']		= $this->input->post('kode_program');
			$insert['skpd']			= $this->input->post('kode_skpd');
			$insert['rka']			= $this->input->post('kode');
			$insert['jan']			= $this->input->post('jan');
			$insert['feb']			= $this->input->post('feb');
			$insert['mar']			= $this->input->post('mar');
			$insert['apr']			= $this->input->post('apr');
			$insert['mei']			= $this->input->post('mei');
			$insert['jun']			= $this->input->post('jun');
			$insert['jul']			= $this->input->post('jul');
			$insert['ags']			= $this->input->post('ags');
			$insert['sep']			= $this->input->post('sep');
			$insert['okt']			= $this->input->post('okt');
			$insert['nov']			= $this->input->post('nov');
			$insert['des']			= $this->input->post('des');			
			$id_kode				= $this->input->post('kode_anggaran');
			$insert['tipe_kode']	= 1;
			
			$query = $this->Rka_model->inserta($insert, $this->input->post('kode'));
			$this->session->set_flashdata('success','<div class="alert alert-info fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "ANGGARAN KAS MURNI" baru telah berhasil ditambahkan.</div>');
			redirect('anggaran-kas/murni/belanja/'.$id_kode);
		}
	...
	public function add() {
		...
		$container['sidebar']['view']						= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu']		= 62;	
		$container['content']['view']						= 'anggaran_kas/murni/belanja_langsung/add';
			...
			$query = $this->Rka_model->inserta($insert);
			$this->session->set_flashdata('success','<div class="alert alert-info fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "ANGGARAN KAS MURNI" baru telah berhasil ditambahkan.</div>');
			redirect('anggaran-kas/murni/belanja/'.$id_kode);
		}
	...
	public function deletes() {
		$admin_log 	= $this->auth->is_login_admin(); {
			...
			redirect('anggaran-kas/murni/sub/'.$sub->rka_rincian, 'refresh');
		}
	}
	...
	public function preview() {	
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 62;
		$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/preview';
		$header['admin_log']							= $admin_log;
	...
	public function hasil() {	
		...
		$this->load->view('anggaran_kas/murni/belanja_langsung/hasil', $laporan);
	}
	...
	public function tim() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 62;
			$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/tim';
	...
	public function anggaran_kas() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 62;
			$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/viewa';
			
			$rka = $this->Rka_model->get('rka.kode, rka.anggaran_kode, skpd.skpd_kode, skpd.skpd_nama, anggaran.kegiatan as id_kegiatan, anggaran.tahun, program.program as program_nama, akun.no as id_akun, kelompok.no as id_kelompok, jenis.no as id_jenis, obyek.no as id_obyek, rincian.no as id_rincian, rincian.rincian_nama as nama_rincian,', array('rka.anggaran_kode'=>$this->uri->segment(4)));	
			$container['content']['dataset']['id_kode']		= $rka->kode;
			$container['content']['dataset']['id_anggaran']	= $rka->anggaran_kode;
			$container['content']['dataset']['nama_skpd']	= $rka->skpd_nama;
			$container['content']['dataset']['skpd_']		= $rka->skpd_kode;
			$container['content']['dataset']['program']		= $rka->program_nama;
			$container['content']['dataset']['kegiatan']	= $rka->id_kegiatan;
			$container['content']['dataset']['akun']		= $rka->id_akun;
			$container['content']['dataset']['kelompok']	= $rka->id_kelompok;
			...
			$container['content']['dataset']['rincian']		= $rka->nama_rincian;
			$container['content']['dataset']['rincian_']	= $rka->id_rincian;
			$container['content']['dataset']['tahun_']		= $rka->tahun;
	...
	public function previewa() {	
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 62;
		$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/previewa';
		...
		$container['content']['dataset']['rka']			= $this->input->post('rka');
		$container['content']['dataset']['anggaran']	= $this->input->post('anggaran');
		$header['admin_log']							= $admin_log;
		...
		$sess_laporan = array(
			...
			'laporan_rka'  		=> $this->input->post('rka'),
			'laporan_anggaran' 	=> $this->input->post('anggaran')
			);
	...
	public function hasila() {	
		...
		$laporan['laporan_rka']			= $sess_laporan['laporan_rka'];
		$laporan['laporan_anggaran']	= $sess_laporan['laporan_anggaran'];
		...
		if ($laporan['laporan_jenis'] == '1'){
			$this->load->view('anggaran_kas/murni/belanja_langsung/hasila', $laporan);
		} else {
			$this->load->view('laporan/rencana_kerja/hasil_perubahan', $laporan);
		}
	...
	public function dpa() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 62;
			$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/viewd';
	...
	public function previewd() {	
		$admin_log = $this->auth->is_login_admin();
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 62;
		$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/previewd';
		$container['content']['dataset']['tanggal']		= $this->input->post('tanggal');
		$container['content']['dataset']['tahun']		= $this->input->post('tahun');
		$container['content']['dataset']['skpd_kode']	= ($this->input->post('skpd_kode'))?$this->input->post('skpd_kode'):'semua';
		$container['content']['dataset']['kecamatan_kode']	= ($this->input->post('kecamatan_kode'))?$this->input->post('kecamatan_kode'):'semua';
	...
	public function hasild() {	
		...
		if ($laporan['laporan_jenis'] == '1'){
			$this->load->view('anggaran_kas/murni/belanja_langsung/hasild', $laporan);
		} else {
			$this->load->view('laporan/rencana_kerja/hasil_perubahan', $laporan);
		}
	...
	public function belanja_tidak_langsung() {
		...
		$waktuSekarang = date("Y-m-d H:i:s");
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			redirect('anggaran-kas/murni/#warningEntri', 'refresh');
		} else {
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 62;
			$container['content']['view']					= 'anggaran_kas/murni/belanja_tidak_langsung/add';
			$container['content']['dataset']['tahun']		= $this->Tahun_model->grid_all('tahun', 'tahun', 'ASC', '', '', array('status'=>'1'));
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'anggaran_kas/murni/belanja_langsung/transfer';
					if ($admin_log['level_kode'] == 5 || $admin_log['level_kode'] == 4){
						$skpd = $this->Skpd_model->get('skpd_kode, skpd_nama', array('skpd_kode' => $admin_log['skpd_kode']));
						$container['content']['dataset']['skpd_kode']	= $skpd->skpd_kode;
						$container['content']['dataset']['skpd_nama']	= $skpd->skpd_nama;
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				if ($this->input->post('apbd_kab') <= $sisa_anggaran){
					...
					redirect('anggaran-kas/murni/#successTransfer', 'refresh');
				} else {
					redirect('anggaran-kas/murni/#warningAPBD', 'refresh');
				}
