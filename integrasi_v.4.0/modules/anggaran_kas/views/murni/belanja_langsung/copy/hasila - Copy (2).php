		...
		for ($i=1; $i <= 2; $i++){
			...
			foreach ($this->Anggaran_model->grid_all('1', 'urusan.kode as urusan_kode, urusan.nomor as urusan_nomor, urusan.urusan as urusan_nama', 'urusan.nomor', 'ASC', '', '', $where_jenis, '', 'anggaran_bl.urusan_kode') as $row){
				...
				if ($this->Anggaran_model->count_bl($where_urusan) > 0){
					...
					$pdf->SetFillColor(255,255,255);
					$left = $pdf->GetX();
					$pdf->Cell(20,$h,"$i.$row->urusan_nomor",1,0,'L',true);
					...
					$pdf->SetFillColor(255,255,255);
					$left = $pdf->GetX();
					$pdf->Cell(20,$h,"$i.$row->urusan_nomor.$subrow->skpd_nomor",1,0,'L',true);
					...
					$where_skpd['anggaran.skpd_kode'] 		= $subrow->skpd_kode;
						...
						if ($this->Anggaran_model->count_bl($where_program) > 0){
							...
							$pdf->SetX($left += 20); $pdf->Cell(85, $h, "Alokasi Belanja Langsung", 1, 0, 'L',true);
							$pdf->SetX($left += 85); $pdf->Cell(18, $h, "", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "", 1, 1, 'R',true);
							...
							$pdf->SetX($left += 85); $pdf->Cell(18, $h, "$subsubrow->subjumlah_apbdkab", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$subsubrow->subjumlah_apbdprov", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$subsubrow->subjumlah_apbn", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$subsubrow->subjumlah_lainnya", 1, 0, 'R',true);
							$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$subsubrow->subjumlah_total", 1, 1, 'R',true);
							...
							foreach ($this->Anggaran_model->grid_all('1', 
							'anggaran.kegiatan, anggaran.alamat, anggaran_bl.hp_ukur, anggaran_bl.hp_target, 
							anggaran_bl.hp_satuan, anggaran_bl.kh_ukur, anggaran_bl.kh_target, anggaran_bl.kh_satuan, 
							anggaran_bl.hk_ukur, anggaran_bl.hk_target, anggaran_bl.hk_satuan, anggaran_bl.apbd_kab, 
							anggaran_bl.apbd_prov, anggaran_bl.apbn, anggaran_bl.sumberlain, anggaran_bl.perkiraan_maju, 
							sifat.sifat_nama, sumber.sumber_nama, (anggaran_bl.apbd_kab+anggaran_bl.apbd_prov+anggaran_bl.apbn+anggaran_bl.sumberlain) 
							as rencana_total, skpd.skpd_nama as skpd_nama, urusan.urusan as urusan_nama, sasaran.sasaran as sasaran_nama, prioritas.prioritas as prioritas_nama,
							program.program as program_nama, indikator.indikator as indikator_nama', 
							'anggaran.kegiatan', 'ASC', '', '', $where_program, '', 'anggaran.kode') as $kegiatanrow){
								
							$where_anggaran['anggaran_kas.anggaran_kode'] = 10;
								
							foreach ($this->Rka_model->grid_ak('1', 
							'anggaran_kas.anggaran, 
							(anggaran_kas.jan+anggaran_kas.feb+anggaran_kas.mar) as triwulan_1,(anggaran_kas.apr+anggaran_kas.mei+anggaran_kas.jun) as triwulan_2,
							(anggaran_kas.jul+anggaran_kas.ags+anggaran_kas.sep) as triwulan_3, (anggaran_kas.okt+anggaran_kas.nov+anggaran_kas.des) as triwulan_4',
							'anggaran.kegiatan', 'ASC', '', '', $where_anggaran, '', 'anggaran_kas.kode') as $kasrow){	
							...
							if ($kegiatan_nomor < 10) {
								...
								$pdf->SetX($left += 85); $pdf->Cell(18, $h, rupiah2($kasrow->anggaran), 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, rupiah2($kasrow->triwulan_1), 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, rupiah2($kasrow->triwulan_2), 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, rupiah2($kasrow->triwulan_3), 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, rupiah2($kasrow->triwulan_4), 1, 1, 'R',true);
								...
								$pdf->SetX($left += 85); $pdf->Cell(18, $h, "$total_apbd", 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$total_apbdp", 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$total_apbn", 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$total_lain", 1, 0, 'R',true);
								$pdf->SetX($left += 18); $pdf->Cell(18, $h, "$grand_total", 1, 1, 'R',true);
		} } } } } } }
		} else {
			$h = 5;
			$left = 40;
			...
			$pdf->SetX($left += 20); $pdf->Cell(175, $h, "Belum ada data.", 1, 0, 'L',true);
		}
