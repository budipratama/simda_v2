...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="data_table">	
	<tr style="font-weight:bold;">
		<td style="text-align:center;" rowspan="2" colspan="1"><img src="<?php echo base_url('public/dist/img/logo.jpg');?>" width="50" height="50"></td>		
	...
	<?php
	$query_data 	= $this->db->query("SELECT program.program as id_program, program.nomor as no_program, skpd.skpd_nama as id_skpd, skpd.urusan as id_urusan, urusan.urusan as nama_urusan, urusan.jenis as jenis_urusan, skpd.skpd_nomor as no_skpd, skpd.skpd_alamat as alamat_skpd, skpd.skpd_status as status_skpd, anggaran.kegiatan as id_anggaran, anggaran.nomor as no_anggaran, anggaran_bl.hp_ukur as hpu_anggaran, anggaran_bl.hp_target as hpt_anggaran, anggaran_bl.hp_satuan as hps_anggaran, anggaran_bl.kh_ukur as khu_anggaran, anggaran_bl.kh_target as kht_anggaran, anggaran_bl.kh_satuan as khs_anggaran, anggaran_bl.hk_ukur as hku_anggaran, anggaran_bl.hk_target as hkt_anggaran, anggaran_bl.hk_satuan as hks_anggaran FROM rka LEFT JOIN urusan ON urusan.kode=rka.urusan LEFT JOIN skpd ON skpd.skpd_kode=rka.skpd LEFT JOIN program ON program.kode=rka.program LEFT JOIN anggaran ON anggaran.kode=rka.anggaran_kode LEFT JOIN anggaran_bl ON anggaran_bl.kode=rka.anggaran_kode WHERE rka.tipe_kode= '1' AND rka.tahun='".$laporan_tahun."' AND rka.anggaran_kode='".$laporan_anggaran."' GROUP BY rka.kode ORDER BY rka.kode ASC");
	$data_get 			= $query_data->result();
	$jumlah_get			= $query_data->num_rows();
		if ($jumlah_get > 0){
			foreach($data_get as $row_get) {
	?>
	<tr>
		<td style="text-align:left;" colspan="2" >Urusan Pemerintahaan</td>
		<td style="text-align:left;" colspan="2" >: 1.<?php echo $row_get->id_urusan; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->jenis_urusan; ?> <?php echo $row_get->nama_urusan; ?></td>
    </tr>
	<tr>
		<td style="text-align:left;" colspan="2" >Organisasi</td>
		<td style="text-align:left;" colspan="2" >: 1.<?php echo $row_get->id_urusan; ?> . 1.<?php echo $row_get->id_urusan; ?>.<?php echo $row_get->no_skpd; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->id_skpd; ?></td>
    </tr>
	<tr>
		<td style="text-align:left;" colspan="2" >Program</td>
		<td style="text-align:left;" colspan="2" >: 1.<?php echo $row_get->id_urusan; ?> . 1.<?php echo $row_get->id_urusan; ?>.<?php echo $row_get->no_skpd; ?> . <?php echo $row_get->no_program; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->id_program; ?></td>
    </tr>
	<tr>
		<td style="text-align:left;" colspan="2" >Kegiatan</td>
		<td style="text-align:left;" colspan="2" >: 1.<?php echo $row_get->id_urusan; ?> . 1.<?php echo $row_get->id_urusan; ?>.<?php echo $row_get->no_skpd; ?> . <?php echo $row_get->no_program; ?> . <?php echo $row_get->no_anggaran; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->id_anggaran; ?></td>
    </tr>
	<tr><td style="text-align:left;" colspan="8" >Lokasi Kegiatan &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; : &nbsp; <?php echo $row_get->alamat_skpd; ?></td></tr>	
	<tr>
	...
	<tr>
		...
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->hpu_anggaran; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="2" ><?php echo $row_get->hpt_anggaran; ?>&nbsp;<?php echo $row_get->hps_anggaran; ?></td>
    </tr>
	<tr>
	...
	<tr>
		...
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->khu_anggaran; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="2" ><?php echo $row_get->kht_anggaran; ?> <?php echo $row_get->khs_anggaran; ?></td>
    </tr>
	<tr>
		<td style="text-align:left;" colspan="2" >HASIL</td>
		<td style="text-align:left; font-size: 9px;" colspan="4" ><?php echo $row_get->hku_anggaran; ?></td>
		<td style="text-align:left; font-size: 9px;" colspan="2" ><?php echo $row_get->hkt_anggaran; ?> <?php echo $row_get->hks_anggaran; ?></td>
    </tr>
	<tr><td style="text-align:left;" colspan="8" >Kelompok Sasaran Kegiatan &nbsp; &nbsp; &nbsp; : <?php echo $row_get->status_skpd; ?></td></tr>
		<?php } } ?>
	...
	for ($i=1; $i < 4; $i++){	
		$query_jenis 	= $this->db->query("SELECT rka_sub.total, SUM(rka_sub.total) as totalRKA FROM rka_sub LEFT JOIN rka ON rka.kode=rka_sub.rka WHERE rka_sub.tipe_kode= '1' AND rka.jenis='".$jenis_belanja[$i]."' AND rka.tahun='".$laporan_tahun."' AND rka.anggaran_kode='".$laporan_anggaran."' GROUP BY rka.kode ORDER BY rka.kode ASC");
...
<!--salah--><?php if($data_jenis): foreach($data_jenis as $task):  echo rupiah1 ($task->totalRKA);  endforeach;  endif; ?>			
			</td>
		</tr>
		<?php			
				...
				if ($jumlah_rincian > 0){
					...
					<?php
						...
						?>
							...
							<?php } } } } } } } } }	?>
		<tr>
			<td style="text-align:left; font-size: 9px;" colspan="4">Keterangan :<br> - Tanggal Pembahasan<br> - Catatan Hasil Pembahasan<br><br><br><br><br><br><br><br><br><br><br><br></td>
			<td style="text-align:center;" colspan="4"><?php echo $laporan_kecamatan; ?>, <?php echo dateIndo($laporan_tanggal); ?>
		<?php
		$query_kp = $this->db->query("SELECT tim_anggaran.nama, tim_anggaran.nip, skpd.skpd_nama as id_skpd
		FROM tim_anggaran
		LEFT JOIN rka ON rka.kode=tim_anggaran.kode
		LEFT JOIN skpd ON skpd.skpd_kode=tim_anggaran.skpd
		WHERE tim_anggaran.dokumen= 'SPM'
		AND tim_anggaran.skpd='".$laporan_skpd."'
		ORDER BY tim_anggaran.kode ASC");
