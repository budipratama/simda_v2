...
<a href="#warningHapus" data-toggle="modal"></a>
		...
		<div class="modal-dialog">
			<div class="modal-content">
			  <div class="modal-header">
				...
				<center><h3 class="modal-title">Rekening Obyek</h3></center>
			  </div>
			<div class="modal-body form">
				<form action="<?php echo site_url('parameter/rekening/addo');?>" id="form" class="form-horizontal" method="post">
	...
	<div class="modal fade" id="editObyek" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-body form">
				<form action="<?php echo site_url('parameter/rekening/edito');?>" id="form1" class="form-horizontal" method="post">
    ...
    function add_obyek() {
		...
		$('.modal-title').text('Tambah Rekening Obyek'); // Set title to Bootstrap modal title
    }
    ...
    function edit_obyek(id) {
		...
		$.ajax({
			url : "<?php echo site_url('parameter/rekening/ajax_obyek/')?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data)
			{
				$('[name="kode"]').val(data.kode);
				$('[name="ddd_kode"]').val(data.kd_rek_4);
				$('[name="eee_kode"]').val(data.nm_rek_4);
				$('[name="jenis"]').val(data.kd_rek_3);
				$('[name="status"]').val(data.status);
				$('#editObyek').modal('show'); // show bootstrap modal when complete loaded
				$('.modal-title').text('Edit Rekening Obyek'); // Set title to Bootstrap modal title				
			},
