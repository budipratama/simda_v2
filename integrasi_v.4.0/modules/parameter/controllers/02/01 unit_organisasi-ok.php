...
class Unit_organisasi extends CI_Controller {
	...
	public function addsub()
	{
		...
		$this->form_validation->set_rules('misi_kode', 'Tipe', 'trim|required|xss_clean');
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$insert['tipe']			= $this->input->post('misi_kode');
	...
	public function tampil_combobox_tujuan_by_misi(){
		...
		if ($misi_kode){
			$data_tujuan = $this->Urusan_model->grid_all('urusan.kode, urusan.urusan', 'urusan.urusan', '', '', '', array('urusan.tipe'=>$misi_kode));			
			echo '<label class="control-label col-md-2" for="tujuan_kode">Bidang :</label>';
			echo '<div class="col-md-10">';
				combobox('db', $data_tujuan, 'tujuan_kode', 'kode', 'urusan', '', 'show_form_sasaran_by_tujuan();', 'Pilih Tujuan', 'class="select2_category form-control" required="required"');
			echo '</div>';
		} else {
	...
	public function tampil_combobox_sasaran_by_sasaran(){
		$sasaran_kode = $this->uri->segment(4);
		if ($sasaran_kode){
			$data_sasaran = $this->Bidang_sub_model->grid_all('bidang_sub.kode, bidang_sub.bidang_sub', 'bidang_sub.bidang_sub', '', '', '', array('bidang_sub.bidang_unit'=>$sasaran_kode));
			echo '<label class="control-label col-md-2" for="urusan_kode">Sub Unit :</label>';
			echo '<div class="col-md-10">';
				combobox('db', $data_sasaran, 'urusan_kode', 'kode', 'bidang_sub', '', '', 'Pilih Sub Unit', 'class="select2_category form-control" required="required"');
			echo '</div>';
		} else {
			echo '<label class="control-label" for="urusan_kode">Sasaran<span class="required">*</span> :</label>
				<select class="select2_category form-control" name="urusan_kode" id="urusan_kode" data-placeholder="Pilih Sasaran" required="required"></select>';
