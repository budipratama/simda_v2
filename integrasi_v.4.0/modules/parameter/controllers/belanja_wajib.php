...
class Belanja_wajib extends CI_Controller {
	...
	public function index()	{
		$admin_log 	= $this->auth->is_login_admin();
		{
			...
			$container['belanja'] 							= $this->Belanja_wajib_model->get_list();
	...
	public function add($id) {
			...
			$container['content']['view']					= 'parameter/belanja_wajib/add';			
		{
			if($this->form_validation->run() == FALSE){
				...
				if($this->Belanja_wajib_model->insert($container)){
					$this->session->set_flashdata('success','<div class="alert fresh-color alert-info fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "BELANJA WAJIB & MENGIKAT" telah berhasil ditambahkan</div>');
	...
	//	$where_kelompok = 'kode IN (\'16\',\'21\',\'22\')';
		...
		if ($akun_kode){
			...
			combobox('db', $data_kelompok, 'bbb_kode', 'kode', 'nm_rek_2', '', 'show_form_kelompok_by_jenis();', 'Pilih Kelompok ...', 'class="form-control" required');
		} else {
