...
class Rekening_akrual extends CI_Controller {
	...
	public function kelompok($id){
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/kelompok';
	...
	public function jenis($id){
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/jenis';			
	...
	public function addj($id){
        ...
        $this->form_validation->set_rules('eee_kode','Saldo_normal','trim|xss_clean');
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/addj';
		{
			if($this->form_validation->run() == FALSE){
				
				$where_saldo								= 'tipe_sort IN (\'3\',\'4\')';
				$container['saldo_']						= 'Debet';
				$container['akun']							= $this->Akun_model->get_list($id);
				$container['kelompok']						= $this->Kelompok_model->get_list($id);
				...
				if($this->Jenis_model->insert($container)){
					...
					redirect('parameter/rekening-akrual/#successInsert', 'refresh');
				}
			}
		}
    }
		
	public function editj1($id){
		$admin_log 	= $this->auth->is_login_admin();
		$this->form_validation->set_rules('aaa_kode','Akun','trim|required|xss_clean');
        $this->form_validation->set_rules('bbb_kode','Kelompok','trim|xss_clean');
        $this->form_validation->set_rules('ccc_kode','No','trim|xss_clean');
        ...
        $this->form_validation->set_rules('eee_kode','Saldo_normal','trim|xss_clean');
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/editj';
		{
			if($this->form_validation->run() == FALSE){
	...
	public function editj($id){
		$admin_log 	= $this->auth->is_login_admin();
		{
			...
			if($this->form_validation->run() == FALSE){	
				...
				if($this->Jenis_model->edit_task($id,$container)){
					$this->session->set_flashdata('task_updated', 'Your task has been updated');
					redirect('parameter/rekening_akrual/'.$list_id.'');
				}
			}
		}
    }
	...
	public function obyek($id){
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/obyek';
	...
	public function rincian($id){
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/rincian';
	...
	public function editr($id){
		$admin_log 	= $this->auth->is_login_admin();
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/editr';
	...
	public function updater()
	{
		...
		if($this->form_validation->run() == FALSE)
		{
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/rekening_akrual/edit';
			...
			redirect('parameter/rekening_akrual', 'refresh');
		}
	}
	...
	function datatable() {
		...
		->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, jenis.kode, jenis.akun, jenis.kelompok, jenis.no, jenis.jenis_nama FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, jenis WHERE akun_sort IN (\'2\') ORDER BY jenis.kode ASC) jenis');
	...
	function get_buttons ($tipe, $id) {
		...
		$html .= '<a href="'. site_url($ci->uri->segment(1) . '/rekening_akrual/detail/'.$id) .'" class="btn btn-sm btn-info" title="Detail"><i class="fa fa-file-text"></i></a>';
		$html .= '<a href="'. site_url($ci->uri->segment(1) . '/rekening_akrual/edit/'.$id) .'" class="btn btn-sm btn-warning" title="Ubah"><i class="fa fa-pencil"></i></a>';
		$html .= '<a href="'. site_url($ci->uri->segment(1) . '/rekening_akrual/delete/'.$id) .'" class="btn btn-sm btn-danger" title="Hapus" data-placement="left" onclick="return confirm(\'Apakah anda yakin? \nAkan menghapus data\');"><i class="fa fa-trash-o"></i></a>';
		$html .= '</div>';
		return $html;
	}
	...
	public function add() {
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 60;
		$container['content']['view']					= 'parameter/rekening_akrual/add';
		$container['content']['dataset']				= '';
		
		$where_akun											= 'akun_sort IN (\'2\')';
		$where_saldo										= 'tipe_sort IN (\'3\',\'4\')';
		$container['content']['dataset']['akun']			= $this->Akun_model->grid_alli('kode, akun_nama', 'akun_nama', '', '', '', $where_akun);
		$container['content']['dataset']['saldo']			= $this->Tipe_model->grid_all('tipe_kode, tipe_nama', 'tipe_nama', '', '', '', $where_saldo);
	...
	public function insert() {
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$insert['no']			= $this->input->post('ccc_kode');
