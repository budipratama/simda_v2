...
class Rekening_akrual extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Rekening_akrual_model');
	...
	public function index1()	{
		$admin_log 	= $this->auth->is_login_admin();
		{
			...
			$container['content']['view']					= 'parameter/rekening_akrual/view';
	...
	public function index() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			...
			$container['content']['view']					= 'parameter/rekening_akrual/view';
				...
				$container['open_tasks'] = $this->Rekening_akrual_model->get_list_tasks(true);
				$container['close_tasks'] = $this->Rekening_akrual_model->get_list_tasks(false);
	...
	function jenisv() {
		...
		$this->datatables->select('nomor, kode, jenis_nama, nama_akun, nama_kelompok')
		->add_column('Actions', $this->get_buttonsj($tipe, '$1'),'kode')
		->search_column('jenis_nama')
        ->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, jenis.kode, jenis.jenis_nama, akun.akun_nama as nama_akun, kelompok.kelompok_nama as nama_kelompok FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, jenis LEFT JOIN akun ON jenis.akun=akun.kode LEFT JOIN kelompok ON jenis.kelompok=kelompok.kode ORDER BY jenis.kode ASC) jenis');
	...
	function obyekv() {
        $this->datatables->select('nomor, kode, obyek_nama, nama_akun, nama_kelompok, nama_jenis')
		->add_column('Actions', $this->get_buttonso($tipe, '$1'),'kode')
		->search_column('obyek_nama')
        ->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, obyek.kode, obyek.obyek_nama, akun.akun_nama as nama_akun, kelompok.kelompok_nama as nama_kelompok, jenis.jenis_nama as nama_jenis FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, obyek LEFT JOIN akun ON obyek.akun=akun.kode LEFT JOIN kelompok ON obyek.kelompok=kelompok.kode LEFT JOIN jenis ON obyek.jenis=jenis.kode ORDER BY obyek.obyek_nama ASC) obyek');
        echo $this->datatables->generate();
    }
	...
	function rincianv() {
		$this->datatables->select('nomor, kode, rincian_nama, nama_akun, nama_kelompok, nama_jenis, nama_obyek')
		->add_column('Actions', $this->get_buttonsr($tipe, '$1'),'kode')
		->search_column('rincian_nama')
        ->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, rincian.kode, rincian.rincian_nama, akun.akun_nama as nama_akun, kelompok.kelompok_nama as nama_kelompok, jenis.jenis_nama as nama_jenis, obyek.obyek_nama as nama_obyek FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, rincian LEFT JOIN akun ON rincian.akun=akun.kode LEFT JOIN kelompok ON rincian.kelompok=kelompok.kode LEFT JOIN jenis ON rincian.jenis=jenis.kode LEFT JOIN obyek ON rincian.obyek=obyek.kode
		ORDER BY rincian.rincian_nama ASC) rincian');
        echo $this->datatables->generate();
    }
