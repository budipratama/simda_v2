...
class Bank extends CI_Controller {
	...
	public function index() {
		$admin_log 	= $this->auth->is_login_admin();
		{
			...
			$container['content']['view']					= 'parameter/bank/view';
			
			$container['bank'] 								= $this->Bank_model->get_list();			
			$where_rek										= 'kode IN (\'7\')';
	...
	public function add($id) {
		...
		$this->form_validation->set_rules('rek1_kode','Kd_rek_1','trim|xss_clean');
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 60;
			$container['content']['view']					= 'parameter/bank/add';			
		{
			if($this->form_validation->run() == FALSE){
				...
				$container = array(             
					'kd_bank'		=> $this->input->post('aaa_kode'),
					'nm_bank'		=> $this->input->post('aaa_kode'),
					'no_rekening'	=> $this->input->post('aaa_kode'),
				...
				if($this->Rekening_potongan_model->insert($container)){
					$this->session->set_flashdata('success','<div class="alert fresh-color alert-info fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><strong>Success!</strong> Data "BANK" telah berhasil ditambahkan</div>');
